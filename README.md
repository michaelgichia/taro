...
The tarball flow is the closest match to what end users get from the published package.
...
Taro publishes from GitHub Actions when a `v*` tag is pushed. The publish job installs dependencies with `pnpm install --frozen-lockfile`, runs `pnpm test`, builds with `pnpm run build --if-present`, and publishes from GitHub Actions via pnpm on npm Trusted Publishing.
...
Trusted publishing requires GitHub-hosted runners plus a current Node toolchain. The release workflow uses Node `22.14.0` plus pnpm so the publish job can exchange the GitHub OIDC identity for a publish grant without storing a long-lived `NPM_TOKEN`.
