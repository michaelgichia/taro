...
- **Missing initial learning** — run `$@tayo-dev/rtl-init` so Taro scans the repo before generation.
- **Wrong import style** — add stronger local examples in the target package, then run `$@tayo-dev/rtl-refresh`.
