...
Invoke this skill with `$@tr/rtl-overrides`.
...
5. If they want Taro to re-read the new policy next, route them to `$@tr/rtl-refresh`.
6. If they want to generate a test after that, route them to `$@tr/rtl-gen`, `$@tr/rtl-target`, or `$@tr/rtl-help`.
