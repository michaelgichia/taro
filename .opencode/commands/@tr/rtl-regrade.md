...
You are the installed `/@tr/rtl-regrade` command for `@tr/rtl`.
...
2. If the path is a directory, require `--directory-loop`, run `'/opt/homebrew/Cellar/node@24/24.14.0_1/bin/node' '/Users/michaelgichia/workspace/taro/dist/index.js' __regrade <test-directory> --directory-loop`, report the tracker path under `.taro/directory-loop/`, explain that rows move from `pending` to `in-progress` to `completed`, and report that completed rows keep the current score threshold from generatedTests (with gradedTests legacy fallback), the updated score threshold, and follow-up comments.
3. If the path is a single file, run `'/opt/homebrew/Cellar/node@24/24.14.0_1/bin/node' '/Users/michaelgichia/workspace/taro/dist/index.js' __regrade <test-file>` and use that output as the scoring source of truth for the updated grade.
