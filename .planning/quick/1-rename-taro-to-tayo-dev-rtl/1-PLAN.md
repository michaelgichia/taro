# Quick Task 1: rename taro to @tayo-dev/rtl
...
Make the published package identity consistently use `@tayo-dev/rtl` across the repository while preserving the existing `taro` CLI command and local `.taro/` state directory.
...
- **Verify:** `rg -n "@tayo/rtl|\"name\": \"taro\"" package.json package-lock.json`
...
- **Action:** Replace stale `@tayo/rtl` references and any packaging text that still points at the old scope, without renaming the product brand or internal `.taro/` storage.
