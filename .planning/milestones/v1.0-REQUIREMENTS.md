# Requirements Archive: v1.0 Tayo v1.0
...
| Feature | Reason |
|---------|--------|
| Test execution | Tayo generates tests only; users run them with their own test runner |
| Component modification | Tayo reads components; does not modify them |
| Accessibility remediation | Tayo flags gaps but does not fix components |
| E2E test generation | Tayo outputs RTL only; Playwright is internal only |
| Live recording | Tayo consumes exports; Chrome Recorder handles recording |
| Test maintenance | Tayo generates on demand; no automatic updates |
| Mock server setup | Tayo mocks at hook/module boundary only |
| Non-RTL paradigms | Tayo is opinionated about RTL; Enzyme/Cypress out of scope |
| Cross-framework | React only for v1 |
| Quality guarantees | Tayo scores output but final judgment belongs to engineer |
