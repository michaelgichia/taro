...
- [x] **INPUT-01**: User can run `tayo generate <file>` against a Testing Library Recorder JS export and use the same `--dry-run`, `--output`, and `--force` flow they use for JSON input.
...
- [x] **QUERY-03**: User does not receive invented selector fallbacks such as fake `data-testid` queries when Tayo cannot recover trustworthy evidence from the recorder or live DOM.
...
- [x] **VERIFY-03**: User sees matching CLI help, README guidance, and examples for JSON and JS input support, including the fact that recorder JS is treated as a baseline artifact Tayo transforms.
...
- **AUTHOR-01**: User can review and refine a generated test interactively before Tayo writes the file.
...
| Non-React framework support or browser-E2E output | Tayo still targets React Testing Library generation, not Playwright-style replay |
