...
Run `npm run test:run` after creating all three files. Tests must FAIL (module not found errors are expected and correct — this is the RED phase).
  ...
  <verify>
    <automated>cd /Users/michaelgichia/workspace/taro && npm run test:run -- src/core/js-parser.test.ts src/core/resolver.test.ts src/core/scanner.test.ts 2>&1 | tail -20</automated>
  </verify>
  <done>
