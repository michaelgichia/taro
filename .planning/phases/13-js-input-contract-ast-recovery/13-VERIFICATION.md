...
**Phase Goal:** Users can feed recorder JS exports into the normal generation flow and have Taro recover baseline intent instead of replaying raw transcript code.
...
- The built CLI dry-run still accepts the shipped Add Sale recorder JS fixture through `taro generate`, produces project-shaped RTL output instead of replaying recorder transcript code, and preserves recovered recorder evidence as the baseline for later phases.
...
|---|-------|--------|----------|
| 1 | Recorder JS and Chrome Recorder JSON enter Taro through one shared parsed-input boundary with stable step IDs and source discrimination | ✓ VERIFIED | `src/core/input-loader.ts`, `src/core/input-loader.test.ts`, and Phase 13-01 summary prove the discriminated `ParsedInput` envelope and shared loader path. |
...
| INPUT-02 | ✓ SATISFIED | The built CLI dry-run and `src/cli/commands/generate.test.ts` show generated RTL test output with Taro-owned structure, scoring, and warnings rather than transcript replay. |
