...
<objective>
Help the user install, initialize, refresh, scaffold overrides, and use @taro-test/rtl from Claude Code.
</objective>
...
7. For explicit component targeting, use `/@taro-test/rtl:target` with a component file path and an optional Recorder `.js` export.
8. Tell the user Taro must write the generated test next to the inferred or supplied component when it resolves the owning render target; unresolved boundary drafts fall back next to the recording, and existing files are never overwritten.
