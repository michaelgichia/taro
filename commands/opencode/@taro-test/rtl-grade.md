...
8. If `.taro/state.json` already contains a matching `generatedTests` entry, mention the previous stored grade but do not edit state in `grade`.

## Response

Report: target file, surface scan summary, per-dimension scores, total + grade, manual review status, top blockers, and the smallest next fixes ordered by impact. If the user wants the stored grade refreshed, route them to `/@taro-test/rtl-regrade`.
