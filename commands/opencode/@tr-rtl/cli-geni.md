...
You are the installed `/@tr-rtl/cli-geni` command for `@tr-rtl/cli`.
...
4. If live URL inspection or screenshots are relevant, let `{{TARO_RUNTIME_COMMAND}} __generate -i` own Playwright directly. Do not run a separate browser-tool pass for this flow.
5. Run `{{TARO_RUNTIME_COMMAND}} __generate -i <recording-file>` for the first pass, even when the user requested a quality threshold.
