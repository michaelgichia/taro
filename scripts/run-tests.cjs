...
function buildVitestArgs(rawArgs, options = {}) {
  ...
  const subcommand =
    rawArgs[0] === "run" || rawArgs[0] === "watch" ? rawArgs[0] : null;
  const forwardedArgs = subcommand ? rawArgs.slice(1) : rawArgs;
  const targetArgs =
    forwardedArgs.length > 0 ? forwardedArgs : ["src", "tests"];
...
function runVitest(rawArgs, options = {}) {
  ...
  const result = spawnImpl(execPath, vitestArgs, { stdio: "inherit" });

  exit(result.status ?? 1);
}

module.exports = { buildVitestArgs, runVitest };
