...
} from "#analyzer/visual/element-analyzer.ts";

const mockPage = { $eval: vi.fn() } as unknown as Page;
  ...
  describe("analyzeElementProperties", () => {
    ...
    it("prefers role queries for interactive elements with visible text and supports parent-label lookup", async () => {
      ...
      Object.defineProperty(globalThis, "document", {
        configurable: true,
        value: { querySelector: documentQuerySelectorMock },
      });
      ...
      try {
        const buttonResult = await analyzeElementProperties(
          mockPage,
          "#save-button"
        );
        const textareaResult = await analyzeElementProperties(
          mockPage,
          "#notes"
        );
    ...
    it("uses alt text for images and falls back to a generic role when no good strategy exists", async () => {
      ...
      const imageResult = await analyzeElementProperties(
        mockPage,
        "#hero-image"
      );
      const fallbackResult = await analyzeElementProperties(
        mockPage,
        "#plain-div"
      );
        ...
        .mockImplementationOnce(async (_selector, callback) =>
          callback({
            getAttribute: (name: string) =>
              name === "data-test-id" ? "legacy-test-id" : null,
          } as unknown as Element)
        );
    ...
    it("evaluates label and alt-text callbacks against the DOM when the element has an id", async () => {
      ...
      Object.defineProperty(globalThis, "document", {
        ...
        value: {
          querySelector: vi
            .fn()
            .mockReturnValue({ textContent: "Email Address" }),
        },
      });
