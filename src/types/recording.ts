...
export type SelectorEvidenceKind =
...
export type SemanticMarkerAnchorRelation =
  | "precedes"
  | "follows"
  | "same-target";
...
export type PlannedMarkerAssertionPlacement =
  | { kind: "after-step"; stepId: StepId }
  | { kind: "after-helper"; helperName: string; stepId: StepId };
...
export type SelectorResolutionPhase =
  | "pre-step"
  | "post-step"
  | "fallback-no-replay";
...
export interface ResolvedSelectorResolutionResult extends BaseSelectorResolutionResult {
  status: "resolved";
...
export interface UnresolvedSelectorResolutionResult extends BaseSelectorResolutionResult {
  status: "unresolved";
  outcome:
    | "no-url"
    | "inspection-failed"
...
export type JsStateSafetyStatus =
  | "safe-multi-it"
  | "setup-replay-required"
  | "unknown";
export type JsScenarioGoal =
  | "flow"
  | "validation"
  | "review"
  | "mutation-state";
