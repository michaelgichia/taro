...
vi.mock('#install/options.ts', async () => {
  const actual = await vi.importActual<typeof import('#install/options.ts')>('#install/options.ts')
  return {
    ...actual,
...
import { applyInstallOptions, createInstallCommand, runInstallCommand } from '#cli/commands/install.ts'
import {
...
describe('applyInstallOptions', () => {
  it('registers the runtime and location flags on a command', () => {
    ...
    expect(command.options.map((option) => option.long)).toEqual(
      expect.arrayContaining(['--claude', '--opencode', '--gemini', '--codex', '--all', '--global', '--local'])
    )
  })
})
...
describe('runInstallCommand', () => {
  it('prints a validation error and sets exitCode when option normalization fails', async () => {
    ...
    expect(errorMock).toHaveBeenCalledWith(expect.stringContaining('bad flags'))
