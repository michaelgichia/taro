...
const makeAllActors = (
  overrides: Partial<Record<string, ReturnType<typeof fromPromise>>> = {}
) => ({
...
function runCollectingStates(actor: ReturnType<typeof createActor>): {
  states: string[];
  done: Promise<void>;
} {
  const states: string[] = [];
...
describe("generateMachine", () => {
  describe("initial state", () => {
    it("starts in idle then immediately transitions to validating", () => {
      const states: string[] = [];
      const actor = createActor(createGenerateMachine(makeAllActors()), {
  ...
  describe("error handling — each actor transitions to failed", () => {
    it("transitions to failed when validateFileActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ validateFileActor: throwWith("file not found") })
        ),
        { input: makeMinimalContext() }
      );
    ...
    it("transitions to failed when parseRecordingActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ parseRecordingActor: throwWith("parse error") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when loadStateActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ loadStateActor: throwWith("state load error") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when searchContextActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ searchContextActor: throwWith("search failed") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when analyzeRecordingActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ analyzeRecordingActor: throwWith("analysis failed") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when planGenerationActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ planGenerationActor: throwWith("plan failed") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when writeOutputActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ writeOutputActor: throwWith("write failed") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
    ...
    it("transitions to failed when finalizeActor throws", async () => {
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ finalizeActor: throwWith("finalize failed") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
  ...
  describe("isProfileStale guard", () => {
    ...
    it("assigns refreshProfileActor output to context before analyzingRecording", async () => {
      ...
      expect((capturedContext?.packageProfile as any)?.packagePath).toBe(
        "packages/app"
      );
  ...
  describe("assessingOutput guards", () => {
        ...
        createGenerateMachine(
          makeAllActors({
            generateCodeActor: noopReturn({
              ...
              candidateAssessment: {
                flowCoverage: highFlow,
                scoreResult: highScore,
              },
            }),
            assessOutputActor: noopReturn({
              existingCode: "old()",
              existingAssessment: {
                flowCoverage: lowFlow,
    ...
    it("transitions to done (not failed) when assessOutputActor throws", async () => {
      ...
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({ assessOutputActor: throwWith("assessment error") })
        ),
        { input: makeMinimalContext() }
      );
      await runToFinal(actor);
  ...
  describe("context assignment", () => {
    it("assigns normalizedRecording and defaultOutputPath from parseRecordingActor", async () => {
      ...
      expect((capturedContext?.normalizedRecording as any)?.title).toBe(
        "My Recording"
      );
    });
    ...
    it("assigns hadState, bootstrappedState, and explicitAuthPath from loadStateActor", async () => {
      ...
      expect(
        (capturedContext?.bootstrappedState as any)?.summary.warnings
      ).toEqual(["w1"]);
    });
    ...
    it("assigns earlyAnalyzedRecording and recordingUrl from captureVisualActor", async () => {
      ...
      expect(capturedContext?.recordingUrl).toBe(
        "http://localhost:3000/feature"
      );
      expect(
        (capturedContext?.earlyAnalyzedRecording as any)?.diagnostics
    ...
    it("assigns packageProfile and contextProfileReason from refineProfileActor", async () => {
      ...
      const actor = createActor(
        createGenerateMachine(
          makeAllActors({
            refineProfileActor: noopReturn({
              packageProfile: makePackageProfile("packages/feature", {
                testFileCount: 5,
              }),
    ...
    it("assigns analyzedRecording and visualAuth from analyzeRecordingActor", async () => {
      ...
      expect(
        (capturedContext?.analyzedRecording as any)?.diagnostics
          .removedRedundantClicks
      ).toBe(1);
      expect((capturedContext?.visualAuth as any)?.strategy).toBe(
        "storageState"
      );
    });
    ...
    it("assigns outputPath and boundarySupportPlan from planGenerationActor", async () => {
      ...
      expect((capturedContext?.boundarySupportPlan as any)?.warnings).toEqual([
        "boundary warning",
      ]);
      expect(
        (capturedContext?.boundarySupportPlan as any)?.requiresReview
      ).toBe(true);
    });
    ...
    it("assigns resolvedJsGeneration from resolveSelectorsActor", async () => {
      ...
      expect((capturedContext?.resolvedJsGeneration as any)?.warnings).toEqual([
        "some warning",
      ]);
    });
    ...
    it("assigns generatedCode, boundaryPolicyWarnings, and candidateAssessment from generateCodeActor", async () => {
      ...
      expect(capturedContext?.generatedCode).toBe(
        'describe("suite", () => {})'
      );
      expect(capturedContext?.boundaryPolicyWarnings).toEqual([
        "policy-warning",
      ]);
    ...
    it("assigns existingCode, outputResolution, and shouldOverwrite from assessOutputActor (shouldWrite path)", async () => {
      ...
      expect(capturedContext?.outputResolution).toEqual(
        makeOutputResolution({ outputCode: "merged()", shouldWrite: true })
      );
        ...
        createGenerateMachine(
          makeAllActors({
            generateCodeActor: noopReturn({
              ...
              boundaryPolicyWarnings: [],
              candidateAssessment: {
                flowCoverage: makeFlowCoverage(0, 0),
                scoreResult: makeScore(30, "C"),
              },
            }),
            assessOutputActor: noopReturn({
              existingCode: "existing()",
