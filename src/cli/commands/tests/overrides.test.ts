...
const {
  ...
  getProjectStatePathMock: vi.fn((projectRoot: string, ...segments: string[]) => {
...
import { createOverridesCommand, overridesCommandInternals } from '#cli/commands/overrides.ts'
...
describe('overridesCommandInternals.buildSuggestedOverrides', () => {
  it('scaffolds runner, render helper, pinned boundaries, and query hook policy from learned state', () => {
...
describe('createOverridesCommand', () => {
  ...
  it('writes a starter .taro/overrides.json scaffold for the current working directory', async () => {
    ...
    const output = logMock.mock.calls.map(([message]) => String(message)).join('\n')
    expect(output).toContain('Wrote /repo/.taro/overrides.json')
    expect(output).toContain('Scaffolded .taro/overrides.json for 1 package(s)')
  })

  it('refuses to overwrite an existing .taro/overrides.json scaffold without --force', async () => {
    findReadableProjectStatePathMock.mockResolvedValue('/repo/.taro/overrides.json')
