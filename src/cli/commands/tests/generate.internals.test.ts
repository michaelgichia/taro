...
vi.mock("#core/component-score-context.ts", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("#core/component-score-context.ts")>();
  return {
    ...actual,
...
vi.mock("#core/mock-intelligence.ts", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("#core/mock-intelligence.ts")>();
  return { ...actual, analyzeMocks: analyzeMocksMock };
});

vi.mock("#core/recording-intelligence.ts", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("#core/recording-intelligence.ts")>();
  return {
    ...actual,
...
vi.mock("#core/verifier.ts", async (importOriginal) => {
  ...
  return { ...actual, verifySyntax: verifySyntaxMock };
});
...
async function createTempDir(label: string): Promise<string> {
  const dir = await mkdtemp(
    join(tmpdir(), `taro-generate-internals-${label}-`)
  );
  tempDirs.push(dir);
  return dir;
}
...
describe("generateCommandInternals", () => {
  it("resolves imported files, rebases render helpers, and flushes findings", async () => {
    ...
    ).toEqual(expect.objectContaining({ importPath: "../tests/render-utils" }));
    expect(
      generateCommandInternals.rebaseRenderHelperImportPath({
        outputPath,
        projectRoot,
    ...
    expect(() => generateCommandInternals.flushFindings(findings)).toThrow(
      ProcessExitSignal
    );
  ...
  it("collects coverage tokens and compares output assessments", () => {
    ...
    expect(
      generateCommandInternals.collectStepCoverageTokens(
        makeStep({ action: "navigate" })
      )
    ).toEqual({ measurable: false, primary: [], secondary: [] });
    ...
    expect(
      ...
      { line: 12, method: "getByRole", quality: "fragile", query: "getByRole" },
    ]);
...
`
      ...
      { method: "getByText", quality: "fragile", query: "getByText(" },
    ]);
...
`,
    ...
    expect(resolution.outputCode).toContain(
      "it('covers the main example flow'"
    );
...
`,
      existingAssessment: {
        ...
        scoreResult: makeScoreResult({ total: 60, grade: "C" }),
      },
      existingCode: `
describe('Example flow', () => {
  it('is stale', async () => {
...
`,
    ...
    expect(resolution.mergeApplied).toBe(false);
    expect(resolution.outputCode).toContain(
      "it('covers the full example flow'"
    );
  ...
  it("finds repo context matches and resolves package/render-target context", async () => {
    ...
    await writeFile(
      join(projectRoot, "not-a-dir.txt"),
      "Review Example Flow",
      "utf-8"
    );
    await writeFile(
    ...
    expect(
      generateCommandInternals.collectVisualElementContextTerm({
        element: { ariaLabel: "Review Example Flow" },
      } as any)
    ...
    expect(matches.map((match) => match.filePath)).not.toContain(
      "packages/example-app/src/Huge.tsx"
    );
    expect(matches.map((match) => match.filePath)).not.toContain(
    ...
    expect(
      ...
      reason:
        "packages/example-app/src/FeatureFlow.tsx matched recording text evidence",
    });
    ...
    ).toEqual({ profile: resolvedProfile, reason: null });
    ...
    ).toEqual({ profile: currentProfile, reason: null });
  ...
  it("reports marker diagnostics, cleanup, score hints, and helper fallbacks", () => {
    ...
    expect(generateCommandInternals.buildMarkerReviewDiagnostics(null)).toEqual(
      { canonicalRecoveries: 0, placementConflicts: 0, placementCorrections: 0 }
    );
    ...
    expect(
      generateCommandInternals.buildMarkerReviewDiagnostics(suitePlan)
    ).toEqual({
    ...
    expect(stderrSpy.mock.calls.join("\n")).toContain(
      "MKR-01 canonical-copy marker=js-step-2"
    );
    expect(warnSpy.mock.calls.flat().join("\n")).toContain(
      "MKR-02 placement-correction marker=js-step-2"
    );
    expect(warnSpy.mock.calls.flat().join("\n")).toContain(
      "MKR-03 unresolved-marker marker=js-step-3"
    );
    ...
    generateCommandInternals.emitScoreHints(
      ...
      [
        { message: "mocked a provider", suggestion: "keep providers real" },
      ] as any
    );
    expect(stderrSpy.mock.calls.join("\n")).toContain("getByTestId queries");
    expect(stderrSpy.mock.calls.join("\n")).toContain("Add specific matchers");
    expect(stderrSpy.mock.calls.join("\n")).toContain(
      "Split into multiple it() blocks"
    );
    expect(warnSpy.mock.calls.flat().join("\n")).toContain(
      "Boundary: mocked a provider"
    );
    ...
    expect(stderrSpy.mock.calls.join("\n")).toContain(
      "Recording cleanup: 1 redundant click(s)"
    );
    ...
    expect(
      generateCommandInternals.buildMarkerCoverageSummary({
        analyzedRecording: {
          diagnostics: {
            preservedSemanticMarkers: 1,
            unresolvedSemanticMarkers: 2,
          },
        },
        suitePlan: null,
      } as any)
    ).toEqual({ detected: 3, emitted: 0, unresolved: 2 });
    ...
    expect(
      generateCommandInternals.toItGroups(
        {
          intentGroups: [
            { name: "learned group", steps: [makeStep({ id: "js-step-9" })] },
          ],
          steps: [],
        } as any,
        "fallback"
      ...
      {
        name: "learned group",
        steps: [expect.objectContaining({ id: "js-step-9" })],
      },
    ]);
    expect(
      generateCommandInternals.toItGroups(
        { intentGroups: [], steps: recording.steps } as any,
        ""
      )
    ).toEqual([{ name: "recorded flow", steps: recording.steps }]);
    expect(generateCommandInternals.isQueryDescriptor(null)).toBe(false);
    expect(
      generateCommandInternals.isQueryDescriptor({ method: "getByRole" })
    ).toBe(true);
    expect(
    ...
    ).toEqual(expect.objectContaining({ warnings: ["first", "second"] }));
        ...
        [
          {
            ...
            markerAssertions: [],
            steps: [
              makeStep({ semanticMarkerLink: { markerStepId: "js-step-2" } }),
            ],
          },
          {
            helperRefs: [],
            ...
            steps: [
              makeStep({
                unresolvedSemanticMarker: { markerStepId: "js-step-3" },
              }),
            ],
          },
        ] as any,
        ...
        makeRecording({
          baseline: {
            ...
            queries: [
              ...
              {
                name: "http://localhost:3001/example",
                target: "Open Example Flow",
              },
            ],
          },
    ...
    expect(
      generateCommandInternals.toProjectRelativePath(
        "/repo",
        "/repo/src/FeatureFlow.tsx"
      )
    ).toBe("src/FeatureFlow.tsx");
    expect(
      generateCommandInternals.toProjectRelativePath(
        "/repo",
        "/tmp/playwright/.auth/user.json"
      )
    ).toBe("playwright/.auth/user.json");
    expect(
      generateCommandInternals.toProjectRelativePath("/repo", "/repo")
    ).toBe(".");
  });
  ...
  it("summarizes mock analysis, audits boundary policy, and scores render targets", async () => {
    ...
    generateCommandInternals.summarizeMockAnalysis({
      ...
      instabilityWarnings: [
        { file: "src/FeatureFlow.test.tsx", reason: "flaky timing" },
      ],
      mutationLifecycles: [
        { file: "src/FeatureFlow.test.tsx", stages: ["idle", "success"] },
      ],
      packagePath: "packages/example-app",
      preferredSharedMocks: { "@/orders/api": "@/tests/mocks/orders-api" },
      recommendations: [
        { count: 2, kind: "shared-mock", target: "@/orders/api" },
      ],
      repeatedTargets: ["@/orders/api"],
      source: "package-profile",
    } as any);
    expect(stderrSpy.mock.calls.join("\n")).toContain(
      "Mock analysis: package=packages/example-app"
    );
    expect(stderrSpy.mock.calls.join("\n")).toContain("Shared mock preference");
    expect(warnSpy.mock.calls.flat().join("\n")).toContain(
      "forbidden targets @/legacy/api"
    );
    expect(warnSpy.mock.calls.flat().join("\n")).toContain(
      "Mock stability: flaky timing"
    );

    discoverBoundaryImportsFromSourceMock.mockResolvedValue([
      { guardrailReason: "repo-owned-ui-wrapper", target: "@/ui/Modal" },
    ]);
    ...
    expect(generateCommandInternals.applyRepoRenderTarget(plan, null)).toBe(
      plan
    );
    expect(
  ...
  it("captures visuals, analyzes mocks defensively, finalizes output, and resolves JS generation edges", async () => {
    ...
    findVisualCaptureCandidatesMock.mockReturnValue([
      { reason: "dialog-state", selector: "#save" },
    ]);
    await generateCommandInternals.maybeCaptureVisualState({
      analyzedRecording: { steps: [] },
    ...
    expect(captureVisualStateMock).toHaveBeenLastCalledWith(
      ...
      expect.objectContaining({ reason: "dialog-state", selector: "#save" })
    );
    ...
    expect(captureVisualStateMock).toHaveBeenLastCalledWith(
      "http://localhost:3001/example",
      expect.objectContaining({ reason: "page-context" })
    );

    analyzeMocksMock.mockResolvedValueOnce({ source: "package-profile" });
    expect(
      await generateCommandInternals.maybeAnalyzeMocks("/repo", null)
    ).toEqual({ source: "package-profile" });
    ...
    verifySyntaxMock.mockReturnValueOnce({
    ...
    expect(
      await generateCommandInternals.resolveJsGeneration(
        makeRecording({ baseline: null }),
        itGroups
      )
    ).toEqual({
      itGroups,
    ...
    resolveSelectorMock.mockImplementationOnce(() =>
      unresolvedSelector("js-step-3", "#first-attempt", "first failure", [
        "first failure",
      ])
    );
    resolveSelectorMock.mockImplementationOnce(() =>
    ...
    expect(fallbackResult.recording.steps[0]?.metadata?.query).toEqual(
      fallbackQuery
    );
...
describe("deriveOutputPath", () => {
  ...
  it("colocates when folderPattern is colocated", () => {
    expect(
      deriveOutputPath("/repo/src/components/Button.tsx", "colocated")
    ).toBe(join("/repo/src/components", "Button.test.tsx"));
  });

  it("places in __tests__/ subdirectory when folderPattern is __tests__", () => {
    expect(
      deriveOutputPath("/repo/src/components/Button.tsx", "__tests__")
    ).toBe(join("/repo/src/components/__tests__", "Button.test.tsx"));
  });
