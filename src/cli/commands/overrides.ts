...
function chooseRenderHelper(
  renderHelpers: TaroRenderHelperProfile[]
): TaroPackageOverrides['renderHelper'] | undefined {
  const [renderHelper] = [...renderHelpers].sort((left, right) => {
...
function shouldPinBoundaryProfile(boundary: TaroBoundaryProfile): boolean {
  return (
    boundary.guardrailReason !== null || boundary.confidence === 'high' || boundary.files.length >= 2
  )
}

function buildPackageOverrides(profile: TaroPackageProfile): TaroPackageOverrides {
  ...
  const renderHelper = chooseRenderHelper(profile.renderHelpers)
  if (renderHelper) {
    overrides.renderHelper = renderHelper
  }
  ...
  for (const boundary of [...profile.boundaryProfiles].sort((left, right) => {
    ...
    if (boundary.supportImportPath) {
      preferredBoundaryImplementations[boundary.target] = boundary.supportImportPath
    }
  }
  ...
  if (Object.keys(preferredBoundaryImplementations).length > 0) {
    overrides.preferredBoundaryImplementations = preferredBoundaryImplementations
  }
  if (forbidBoundaryTargets.size > 0) {
    overrides.forbidBoundaryTargets = [...forbidBoundaryTargets].sort((left, right) => {
...
function buildSuggestedOverrides(state: TaroState): TaroOverrides {
  const packageEntries = Object.values(state.packages)
    ...
    .map((profile) => [profile.packagePath, buildPackageOverrides(profile)] as const)
...
function formatOverridesSummary(overrides: TaroOverrides): string[] {
  const packageEntries = Object.entries(overrides.packages ?? {}).sort(([left], [right]) => {
    return left.localeCompare(right)
  })
...
export function createOverridesCommand(): Command {
  ...
  overrides
    .description('Internal runtime-only overrides scaffold generator for Taro package policies')
    .option('--force', 'Overwrite an existing .taro/overrides.json file')
    .option('--stdout', 'Print the generated .taro/overrides.json payload instead of writing it')
    .action(async (options: OverridesCommandOptions) => {
      ...
      const existingOverridesPath = await findReadableProjectStatePath(projectRoot, 'overrides.json')
      ...
      console.log(`${pc.dim('[taro]')} ${existingOverridesPath ? 'Replaced' : 'Wrote'} ${overridesPath}`)
