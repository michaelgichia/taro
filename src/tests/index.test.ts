...
vi.mock("#version.ts", () => ({ TARO_VERSION: "1.2.3" }));
...
describe("src/index.ts", () => {
  it("routes __generate to the internal generate command", async () => {
    ...
    expect(parseGenerateMock).toHaveBeenCalledWith(["recording.js"], {
      from: "user",
    });
  });
  ...
  it("routes __overrides to the internal overrides command", async () => {
    ...
    expect(parseOverridesMock).toHaveBeenCalledWith(["--stdout"], {
      from: "user",
    });
  });
  ...
  it("routes __refresh to the internal refresh command", async () => {
    ...
    expect(parseRefreshMock).toHaveBeenCalledWith(["--verbose"], {
      from: "user",
    });
  });
