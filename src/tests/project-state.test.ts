...
describe("project state helpers", () => {
  it("creates the canonical .taro directory when no state exists", async () => {
    ...
    expect(
      await findReadableProjectStatePath(projectRoot, "conventions.json")
    ).toBeNull();
  });
  ...
  it("reuses the canonical .taro directory when it already exists", async () => {
    ...
    expect(
      await findReadableProjectStatePath(projectRoot, "history.json")
    ).toBe(getProjectStatePath(projectRoot, "history.json"));
  });
  ...
  it("creates and resolves the canonical .taro directory through the sync helpers", async () => {
    ...
    expect(stateDir).toBe(join(projectRoot, ".taro"));
    expect(findReadableProjectStatePathSync(projectRoot, "state.json")).toBe(
      filePath
    );
    expect(
      findReadableProjectStatePathSync(projectRoot, "missing.json")
    ).toBeNull();
  });
});
