...
describe('renderInstallSummary', () => {
  it('formats global and local install targets with asset counts', () => {
    ...
    expect(summary).toContain('No files will be written until confirmation completes.')
  })
})
...
describe('confirmInstallPlan', () => {
  ...
  it('returns false for empty and negative answers, and closes the interface', async () => {
    ...
    await expect(confirmInstallPlan(createPlan([createPlannedTarget()]))).resolves.toBe(false)
  ...
  it('re-prompts on invalid input, logs guidance, and accepts yes', async () => {
    ...
    await expect(confirmInstallPlan(createPlan([createPlannedTarget(), createPlannedTarget()]))).resolves.toBe(true)

    expect(questionMock).toHaveBeenCalledTimes(2)
    expect(questionMock).toHaveBeenLastCalledWith('Proceed with the 2-target install plan? [y/N]: ')
    expect(logSpy).toHaveBeenCalledWith(expect.stringContaining('Answer `y` to continue or `n` to cancel.'))
    expect(closeMock).toHaveBeenCalled()
  })
})
...
describe('renderInstallExecutionResult', () => {
  it('renders installed, updated, repaired, and blocked outcomes with verification and ownership sections', () => {
      ...
      targets: [
        {
          ...
          verification: {
            verificationCommand: '/@taro-test/rtl:help',
            status: 'verified',
            checkedPath: '/Users/tester/.claude/commands/@taro-test/rtl/help.md',
    ...
    expect(output).toContain('external collision at /repo/.codex/manifest.json')
    expect(output).toContain('missing at /repo/.codex/help.md')
    expect(output).toContain('Verification commands:')
    expect(output).toContain('(verified at /Users/tester/.claude/commands/@taro-test/rtl/help.md)')
    expect(output).toContain('(missing /Users/tester/.gemini/commands/help.toml)')
...
describe('renderInstallCancelledMessage', () => {
  it('returns the static cancellation notice', () => {
    expect(renderInstallCancelledMessage()).toBe('Install cancelled. Nothing changed.')
  })
})
