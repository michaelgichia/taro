...
function createTarget(
  ...
  return {
    ...
    runtimeEntrypointPath: '/Users/tester/.claude/commands/@taro-test/rtl/help.md',
...
describe('verifyInstalledRuntime branches', () => {
  beforeEach(() => {
    ...
    execFileMock.mockImplementation((_file, _args, callback) => callback(null, { stdout: '1.0.0\n', stderr: '' }))
  })
  ...
  it('returns missing-installed-assets when required files are absent', async () => {
    accessMock.mockImplementation((path: string) => {
      if (path.includes('install-manifest.json') || path.includes('rtl/help.md')) {
    ...
    expect(result.checkedPath).toBe('/Users/tester/.claude/commands/@taro-test/rtl/help.md')
