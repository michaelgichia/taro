...
import type { InstallSelection, RuntimeLocationSelections } from '#install/types.ts'
...
describe('resolveInstallTargets', () => {
  it('uses process.cwd() when cwd is omitted for local installs', () => {
    ...
    expect(target.runtimeEntrypointPath).toBe('/tmp/package-root/dist/index.js')
  })

  it('uses homedir() when home is omitted for global installs', () => {
    const [target] = resolveInstallTargets(createSelection('claude', 'global'), {
      cwd: '/tmp/project',
      nodePath: '/tmp/node',
      packageRoot: '/tmp/package-root',
    })
