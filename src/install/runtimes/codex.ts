...
const CODEX_SKILL_ASSETS: RuntimeAssetDefinition[] = [
  ...
  ...TARO_REFERENCE_FILES.map((fileName) => ({
    ...
    destinationSegments: ['skills', '@taro-test', 'rtl-generate', 'references', fileName],
  })),
  ...TARO_REFERENCE_FILES.map((fileName) => ({
    id: `generate-i-reference-${fileName.replace(/\.md$/, '')}`,
    kind: 'skill' as const,
    sourceSegments: ['taro', 'references', fileName],
    destinationSegments: ['skills', '@taro-test', 'rtl-generate-i', 'references', fileName],
  })),
  {
    id: 'conventions',
    kind: 'skill',
    sourceSegments: ['agents', 'taro-conventions.md'],
    destinationSegments: ['skills', '@taro-test', 'rtl-conventions', 'SKILL.md'],
