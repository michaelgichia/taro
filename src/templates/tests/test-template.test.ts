...
describe("importBlock", () => {
  it("renders ESM imports with optional helpers, waitFor, and within", () => {
    ...
    expect(result).toContain(
      "import CheckoutForm from '#components/checkout.tsx'"
    );
    expect(result).toContain(
      "import { renderWithProviders } from '#test/render.ts'"
    );
  ...
  it("renders CommonJS imports with default helper import and render fallback", () => {
    const result = importBlock(false, "cjs", {
      runner: "jest",
      renderTarget: { symbol: "App", importPath: "#app.tsx" },
      renderHelper: {
        name: "renderApp",
    ...
    expect(result).toContain(
      "const renderApp = require('#test/render-app.ts').default"
    );
  ...
  it("renders CommonJS userEvent imports and named helpers, plus ESM default helpers", () => {
    ...
    expect(cjs).toContain(
      "const renderCheckout = require('#test/render-checkout.ts').renderCheckout"
    );
    ...
    expect(esm).toContain(
      "import renderCheckout from '#test/render-checkout.ts'"
    );
  ...
  it("omits jest-dom imports when the package registers them globally", () => {
    const esm = importBlock(false, "esm", { jestDomImportPath: null });
    const cjs = importBlock(false, "cjs", { jestDomImportPath: null });
...
describe("stepTemplate", () => {
  it("renders action-specific templates and checkpoint comments", () => {
    expect(
      stepTemplate({ action: "click", query: "screen.getByRole('button')" })
    ...
    expect(stepTemplate({ action: "scroll", query: "container" })).toBe(
      "container.scrollIntoView()"
    );

    expect(
      stepTemplate({ action: "assert", query: "screen.findByText('Saved')" })
    ...
    expect(
      stepTemplate({ action: "keyDown", query: "ignored", value: "{Escape}" })
    ).toBe("await user.keyboard('{Escape}')");

    expect(stepTemplate({ action: "unknown", query: ".legacy-selector" })).toBe(
      "// TODO: unsupported step — original selector: .legacy-selector"
    );
...
describe("assertion helpers", () => {
  it("normalizes async and sync marker assertion matchers and wraps waitFor blocks", () => {
    expect(
      markerAssertionTemplate({ queryExpression: "screen.findByText('Done')" })
    ...
    expect(
      waitForAssertionBlock(["expect(a).toBe('x')", "expect(b).toBe('y')"])
    ).toBe(
      [
        "await waitFor(() => {",
        "  expect(a).toBe('x')",
...
describe("block builders", () => {
  it("renders describe, helper, and multi-it blocks with escaping and setup", () => {
    ...
    expect(
      describeBlock(
        "save user's item",
        ["render(<App />)", "expect(screen.getByText('Saved')).toBeVisible()"],
        true
      )
    ).toContain("it('save user\\'s item', async () => {");

    const multi = describeBlockMultiIt(
      "checkout flow",
      [
        {
          name: "submits order",
        ...
        },
        {
          name: "shows defaults",
          hasUserEvents: false,
          stepLines: ["expect(screen.getByText('Draft')).toBeVisible()"],
        },
      ],
      {
    ...
    expect(multi).toContain(
      "const renderResult = renderWithProviders(<Checkout />)"
    );
    expect(multi).toContain(
      "const openDialog = async (user: ReturnType<typeof userEvent.setup>) => {"
    );
