...
vi.mock("#learner/storage.ts", () => {
  ...
  return { ConventionStore: MockConventionStore, createStore: createStoreMock };
});
...
function convention(
  overrides: Partial<ReturnType<typeof createEmptyConvention>> = {}
) {
  return {
    ...createEmptyConvention(),
    ...overrides,
    naming: { ...createEmptyConvention().naming, ...overrides.naming },
    structure: { ...createEmptyConvention().structure, ...overrides.structure },
    queries: { ...createEmptyConvention().queries, ...overrides.queries },
    matchers: { ...createEmptyConvention().matchers, ...overrides.matchers },
    imports: { ...createEmptyConvention().imports, ...overrides.imports },
  };
}
...
describe("learnConventions", () => {
    ...
    extractConventionsMock.mockImplementation((dir: string) => {
      if (dir.endsWith("/tests")) {
        return convention({
          ...
          queries: { preferred: ["getByRole"], avoided: [] },
          imports: { common: ["vitest"] },
        });
      }
      ...
      if (dir.endsWith("/__tests__")) {
        return convention({
          ...
          queries: { preferred: ["findByText"], avoided: ["getByTestId"] },
          matchers: { common: ["toBeVisible"] },
          imports: { common: ["@testing-library/react"] },
        });
      }

      return convention({ matchers: { common: ["toBeInTheDocument"] } });
    });
  });
  ...
  it("learns conventions from discovered test directories and files, then persists them", () => {
    ...
    expect(extractConventionsMock).toHaveBeenCalledWith(
      join(root, "__tests__")
    );
...
describe("getConventions", () => {
  ...
  it("returns null when no readable conventions database exists", () => {
    ...
    findReadableProjectStatePathSyncMock.mockReturnValue(
