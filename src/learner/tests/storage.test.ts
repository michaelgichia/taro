...
function createDbHarness() {
  ...
  const cacheRows = new Map<string, { value: string; expires_at: string | null }>()
  ...
  const prepare = vi.fn((sql: string) => {
    ...
    if (sql.includes('SELECT value FROM conventions')) {
      return {
        get: vi.fn(() => conventionRow),
      }
    }

    if (sql.includes('SELECT value, expires_at FROM cache')) {
      return {
        get: vi.fn((key: string) => cacheRows.get(key)),
      }
    }
...
describe('ConventionStore', () => {
  ...
  it('creates the database directory and schema during init', () => {
    ...
    expect(mkdirSyncMock).toHaveBeenCalledWith('/repo/.taro', { recursive: true })
    expect(databaseFactoryMock).toHaveBeenCalledWith('/repo/.taro/conventions.db')
  ...
  it('throws when methods are used before initialization', () => {
    ...
    expect(() => store.saveConventions({} as never)).toThrow('Database not initialized')
  ...
  it('sets, gets, expires, deletes, and clears cache entries', () => {
    ...
    harness.setCacheRow('expired', { stale: true }, new Date(Date.now() - 1000).toISOString())
