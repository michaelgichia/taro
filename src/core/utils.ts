import { DIALOG_SCOPED_ACTION_PATTERN } from "#core/constant.ts";
import {
...
export function getSelectorCheckpoint(
  step: NormalizedStep
  ...
  return helper.steps.flatMap((step) => {
    ...
    if (!query) {
      ...
      const fallbackCheckpoint = {
        reason:
          "No trustworthy RTL query evidence was recovered for this step.",
