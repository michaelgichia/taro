...
const REPO_CONTRACT_REASON_CONFIG: Record<
  ...
  { dimension: keyof ScoreDimensions; code: string; weight: number; severity?: 'advisory' | 'blocker' }
> = {
  'helper-assertion': {
...
function extractMatcherName(node: t.CallExpression): string | null {
  ...
  return t.isCallExpression(object) && t.isIdentifier(object.callee, { name: 'expect' })
    ? matcherName
    : null
}
...
function getCallbackFunction(
  ...
  const callback = node.arguments.find((argument) => {
    return t.isFunctionExpression(argument) || t.isArrowFunctionExpression(argument)
  })

  return callback && (t.isFunctionExpression(callback) || t.isArrowFunctionExpression(callback))
    ? callback
    : null
}
...
function collectTestBlockMetrics(
  ...
  walk(t.isBlockStatement(callback.body) ? callback.body : callback.body, (candidate) => {
...
function analyzeTestCode(
  ...
  if (!ast) {
    ...
    return {
      ...
      hasOverrideRenderHelper: /Partial\s*</u.test(code) && /\.\.\.overrides/u.test(code),
  ...
  walk(ast, (candidate) => {
    if (t.isVariableDeclarator(candidate) && t.isIdentifier(candidate.id, { name: 'BASE_PROPS' })) {
...
function normalizeComponentScoreContext(
  ...
  return {
    ...
    componentEventHandlerCount: normalizeCount(input?.componentEventHandlerCount),
...
function calculateAssertionScore(
  ...
  if (recognizedAssertions === 0) {
    ...
    score = 80 + Math.min(Math.max(analysis.strongAssertionCount - 1, 0), 2) * 10
  }
...
function buildBranchCoverageSignal(
  ...
  return {
    ...
    ratio,
    lowCoverage:
      minimumExpectedTestCount > 0 && itCount < minimumExpectedTestCount * 0.5,
    partialCoverage:
      minimumExpectedTestCount > 0 &&
      itCount >= minimumExpectedTestCount * 0.5 &&
      itCount < minimumExpectedTestCount,
  }
}
...
function calculateStructureScoreFromAnalysis(params: {
  ...
  const { analysis, branchCoverage, code, componentContext, repoContractIssues } = params
  let score = 50
...
export function calculateStructureScore(
  ...
  const branchCoverage = buildBranchCoverageSignal(analysis.itCount, componentContext)
...
function collectSignals(params: {
  ...
  const { code, queryResults, boundaryIssueCount, analysis, branchCoverage, missingMockCount } =
    params
  return {
    queryCheckpointCount: countMatches(code, QUERY_CHECKPOINT_REGEX),
    roleQueryCount:
      queryResults.filter(
        (queryResult) => getSupportedTestingLibraryQueryFamily(queryResult.method) === 'ByRole'
      ).length + analysis.roleQueryCountFromCode,
    testIdQueryCount:
      queryResults.filter((queryResult) => isTestIdQueryMethod(queryResult.method)).length +
      analysis.testIdQueryCountFromCode,
  ...
  if (branchCoverage.lowCoverage) {
    reasons.push(
      createReason(
        ...
        'negative',
        25,
        `Only ${analysis.itCount} test block(s) cover a component that implies at least ${branchCoverage.minimumExpectedTestCount} test cases from branches, handlers, and exported utilities.`,
        'blocker'
      )
    )
  } else if (branchCoverage.partialCoverage) {
    reasons.push(
      createReason(
        ...
        'negative',
        12,
        `The suite has ${analysis.itCount} test block(s), but the component surface implies at least ${signals.minimumExpectedTestCount} test case(s) to cover branches, handlers, and exported utilities.`,
        'advisory'
      )
    )
  }
...
function deriveBlockers(reasons: ScoreReason[], limit = 2): string[] {
  const blockerReasons = reasons
    .filter((reason) => reason.impact === 'negative' && reason.severity === 'blocker')
    .sort(compareReasons)
...
export function scoreGeneratedTest(
  ...
  const dimensions: ScoreDimensions = {
    ...
    boundaryIsolation: clampScore(
      calculateBoundaryIsolationScore(code) - mockCompleteness.penalty - incompleteAssetPenalty
    ),
  }
