...
const VOLATILE_SELECTOR_RULES = [
  {
    label: 'dynamic Radix id',
    pattern: /#radix-[\w-]+/i,
  },
  {
    label: 'generated CSS class',
    pattern: /\.css-[\w-]+/i,
  },
  {
    label: 'positional selector',
    pattern: /nth-(?:of-type|child)\(/i,
  },
] as const
...
function parseSupportedQueryMethod(method?: string): SupportedQueryMethodMatch | undefined {
  if (!method) {
    return undefined
  }
  ...
  const normalizedFamily = family ? (`By${family}` as SupportedQueryFamily) : undefined
  if (
    !variant ||
    !normalizedFamily ||
...
export function toSingularAsyncQueryMethod(method?: string): string | undefined {
...
export function getUnsupportedSelectorReason(selector?: string): string | undefined {
  if (!selector) {
    return undefined
  }

  const matchedRules = VOLATILE_SELECTOR_RULES.filter((rule) => rule.pattern.test(selector))
