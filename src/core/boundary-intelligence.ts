...
function isFrameworkCallback(path: NodePath<t.Function>): boolean {
  const parentCall = path.findParent((candidate) => {
    ...
    const callbackArg = candidate.node.arguments.find(
      (argument) => argument === path.node
    );
...
export function analyzeBoundaryIsolation(code: string): BoundaryIssue[] {
  ...
  traverse(ast, {
    ...
    CallExpression(path: NodePath<t.CallExpression>) {
      ...
      if (
        ...
        ["getByRole", "findByRole", "getAllByRole"].includes(
      ...
      if (
        ...
        const target = t.isStringLiteral(targetArg)
          ? targetArg.value
        ...
        const returnedPropertyNames =
          getReturnedPropertyNames(objectExpression);
        ...
        for (const property of objectExpression.properties) {
          if (
            ...
            ((t.isIdentifier(property.key) &&
              /^use[A-Z].*(Query|Mutation)$/u.test(property.key.name)) ||
  ...
  if (
    [...renderedComponents].some((componentName) =>
      LEAF_RENDER_SUFFIX.test(componentName)
    ) &&
    hasButtonQuery &&
    hasHeadingQuery
  ) {
  ...
  if (hasPositionalControlSelection) {
    issues.push({
      ...
      severity: "warning",
      message:
        "Generated test relies on positional indexing from getAllByRole(...)[n].",
      suggestion:
