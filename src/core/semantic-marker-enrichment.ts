  ...
  for (const token of fragmentTokens) {
    const foundIndex = candidateTokens.findIndex(
      (candidateToken, index) =>
        index >= searchIndex && candidateToken.startsWith(token)
    );
    if (foundIndex === -1) {
      return false;
    }
...
function canRecoverCandidate(candidate: SemanticMarkerCandidate): boolean {
  ...
  return Boolean(
    buildRecoveredQuery(
      candidate.query,
      candidate.proofText ?? candidate.target ?? ""
    )
  );
}
...
function scoreRecoveryCandidate(
  ...
  if (
    candidateLower === fragmentLower ||
    !candidateLower.includes(fragmentLower)
  ) {
  ...
  score += Math.max(
    0,
    20 - Math.max(0, candidateText.length - fragment.length)
  );
  score += /[:.!?)]$/.test(candidateText) ? 3 : 0;
  score +=
    candidateText.split(/\s+/).length > fragment.split(/\s+/).length ? 4 : 0;
...
function applyRecoveryToCandidate(
  ...
  const query = candidate.query
    ? buildRecoveredQuery(candidate.query, recovery.toText)
    : undefined;
...
function updateStepCandidate(
  ...
  const nextCandidate = applyRecoveryToCandidate(
    candidate as SemanticMarkerCandidate,
    recovery
  );
  return {
    ...step,
...
function updateBaselineCandidates(
  ...
  return {
    ...baseline,
    semanticMarkerCandidates: baseline.semanticMarkerCandidates?.map(
      (candidate) => {
        const recovery = recoveriesByStepId.get(candidate.stepId);
        return recovery
          ? applyRecoveryToCandidate(candidate, recovery)
          : candidate;
      }
    ),
    itGroups: baseline.itGroups.map((group) => ({
      ...
      steps: group.steps.map((step) =>
        updateStepCandidate(step, recoveriesByStepId)
      ),
    })),
  };
}
...
export async function enrichCanonicalSemanticMarkers(params: {
  ...
  const sourceMatches = params.contextMatches.filter(
    (match) => match.kind === "source"
  );
  ...
  const enrichedMatches = (
    await Promise.all(
      sourceMatches.map(async (match) => {
        const content = await readFile(
          join(params.projectRoot, match.filePath),
          "utf-8"
        ).catch(() => null);
        if (!content) {
          return null;
    ...
    (match): match is SemanticMarkerContextMatch & { strings: string[] } =>
      Boolean(match)
  );
  ...
  return {
    ...params.recording,
    steps: params.recording.steps.map((step) =>
      updateStepCandidate(step, recoveriesByStepId)
    ),
    baseline: updateBaselineCandidates(
      params.recording.baseline,
      recoveriesByStepId
    ),
  };
}
