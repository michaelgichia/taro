  ...
  const elementInfo = await locator.evaluate((el: Element) => {
    ...
    const labelableEl: LabelableElement | null =
      ...
      el instanceof HTMLTextAreaElement
        ? el
        : null;
    const associatedLabelText = labelableEl?.labels
...
function getQueryScope(query: QueryDescriptor): string {
  if (query.raw) {
    const match = query.raw.match(
      /^(.*)\.(?:get|find|query)(?:All)?By[A-Za-z]+\(.+\)$/
    );
    if (match?.[1]) {
      return match[1];
    }
...
function buildScopedQueryExpression(
  ...
  method: string,
  options: { role?: string; target?: string; name?: string }
): string {
...
function buildAsyncQueryDescriptor(
  query: QueryDescriptor,
  options: { method: string; role?: string; target?: string; name?: string }
): QueryDescriptor {
...
function resolveRoleNameAssertion(
  ...
  const accessibleName = normalizeProofText(
    query.name ?? query.target ?? candidate.proofText
  );
...
function resolveVisibleTextAssertion(
  ...
  const proofText = normalizeProofText(
    candidate.proofText ?? query.target ?? candidate.target
  );
...
function resolveFieldContextAssertion(
  ...
  if (isPlaceholderTextQueryMethod(query.method)) {
    ...
    const assertion =
      asyncQuery &&
      buildAssertion(step, candidate, asyncQuery, "placeholder-text");
  ...
  if (
    ...
    FIELD_LABEL_HINT_PATTERN.test(proofText)
  ) {
    const asyncQuery = buildAsyncQueryDescriptor(query, {
      method: "findByLabelText",
      target: proofText,
...
export function resolveSemanticMarkerAssertion(
  ...
  const roleNameResolution = resolveRoleNameAssertion(
    step,
    candidate,
    candidate.query
  );
...
export async function extractDialogState(
  page: Page
  ...
  try {
    const state = await page.evaluate(() => {
      ...
      return {
        role:
          (dialog.getAttribute("role") as "dialog" | "alertdialog" | null) ??
          null,
...
export function urlsMateriallyDiffer(
  ...
  try {
    ...
    const expectedValue =
      `${expected.pathname.replace(/\/+$/, "") || "/"}${expected.search}`.toLowerCase();
    const actualValue =
      `${actual.pathname.replace(/\/+$/, "") || "/"}${actual.search}`.toLowerCase();
    return expectedValue !== actualValue;
  } catch {
    return (
      normalizeComparableText(expectedUrl) !==
      normalizeComparableText(actualUrl)
    );
  }
}

function titlesMateriallyDiffer(
...
async function detectAuthCheckpoint(
  ...
  const bodyAnalysis = await page
    ...
    .evaluate((body: Element, landmarks: string[]) => {
      ...
      const normalizedText = root.innerText
        .replace(/\s+/g, " ")
        .trim()
        .toLowerCase();
      ...
      if (
        /\b(sign in|log in|continue with|single sign-on|sso|password|verification code|2fa|mfa|checkpoint|challenge)\b/i.test(
          combinedCopy
        )
      ) {
      ...
      return { authSignals, matchedLandmarks };
    }, expectedLandmarks)
    .catch(() => ({
  ...
  const pageTitleMismatch = titlesMateriallyDiffer(
    options.expectedTitle,
    options.actualTitle
  );
  const missingExpectedSelector =
    Boolean(options.selector) && options.element === null;
  ...
  const unresolvedPageMismatch =
    routeMismatch &&
    (pageTitleMismatch || missingExpectedSelector || missingExpectedLandmarks);
  const interrupt =
    (authSignals.size > 0 ||
...
async function inspectVisualPage(
  ...
  return { authCheckpoint, dialog, element, pageTitle };
}
...
function buildAuthInterruptSignals(params: {
  ...
  if (
    expected?.landmarks?.length &&
    authCheckpoint.missingLandmarks.length > 0
  ) {
...
function toAuthInterruptedVisualState(params: {
  ...
  const { auth, expected, reason, screenshotPath, selector, snapshot, url } =
    params;
...
function isStartingPointConfirmed(params: {
  ...
  if (
    snapshot.authCheckpoint.routeMismatch ||
    snapshot.authCheckpoint.pageTitleMismatch
  ) {
    return false;
  }
...
async function waitForStartingPoint(params: {
  ...
  selector?: string;
  timeoutMs: number;
}): Promise<{ snapshot: VisualPageSnapshot; startingPointConfirmed: boolean }> {
  ...
  while (Date.now() <= deadline) {
    ...
    if (snapshot.authCheckpoint.interrupt || startingPointConfirmed) {
      return { snapshot, startingPointConfirmed };
    }
  ...
  try {
    while (Date.now() <= deadline) {
      ...
      if (
        ...
        recovery.saveStorageStatePath &&
        !persistedAuthPath
      ) {
        await mkdir(dirname(recovery.saveStorageStatePath), {
          recursive: true,
        });
      ...
      if (isStartingPointConfirmed({ expected, selector, snapshot })) {
      ...
      if (
        ...
        shouldRetryExpectedUrlDuringAuthRecovery({ expected, snapshot })
      ) {
        retryAttemptCount += 1;
        try {
      ...
      await page.waitForTimeout(
        Math.min(PLAYWRIGHT_AUTH_RECOVERY_POLL_MS, remainingMs)
      );
    }
    ...
    return {
      ...
      finalUrl:
        finalSnapshot?.authCheckpoint.reachedUrl ?? initialInterrupt.finalUrl,
      interrupt: initialInterrupt.interrupt,
      matchedLandmarks:
        finalSnapshot?.authCheckpoint.matchedLandmarks ??
        initialInterrupt.matchedLandmarks,
    ...
    return {
      ...
      url,
      warnings: [
        `Playwright authentication could not be completed: ${message}`,
      ],
    };
  }
}
    ...
    if (snapshot.authCheckpoint.interrupt) {
      ...
      if (options.authRecovery?.enabled) {
        const timeoutSec = Math.round(
          (options.authRecovery.timeoutMs ?? 0) / 1000
        );
        console.log(
          pc.dim("[taro]") +
...
async function inspectSelector(
  ...
  try {
    ...
    const element = await readOptionalElementInfo(
      inspectionSession.page,
      cssSelector
    );
    if (!element) {
...
export async function resolveSelector(
  ...
  const unsupportedSelectorReason = getUnsupportedSelectorReason(
    selector.selector
  );
  if (unsupportedSelectorReason) {
    return toUnresolvedSelectorResult(
      ...
      { inspectSource, pageUrl: url, phase }
    );
  }
...
export async function inspectElements(
  ...
  try {
    ...
    for (const selector of selectors) {
      result.set(
        selector,
        await readOptionalElementInfo(inspectionSession.page, selector)
      );
    }
  } catch (error) {
...
function queryToPlaywrightLocator(
  ...
  if (method === "getByRole" && query.role) {
    ...
    return page.getByRole(
      query.role as Parameters<Page["getByRole"]>[0],
      options
    );
  }
  if (method === "getByText") {
...
async function summarizeReplayLocatorState(
  ...
  if (summary.texts.length > 0) {
    parts.push(
      `texts=${summary.texts.map((text) => JSON.stringify(text)).join("|")}`
    );
  }
...
function getSkippedReplaySelector(
  step: NormalizedStep
): SkippedReplaySelector | null {
  ...
  const selectorMeta = step.metadata?.selector as
    | { selector?: string }
    | undefined;
...
function resolveStepLocator(
  ...
  if (step.target) {
    try {
      ...
      return { locator: null, source: "step.target", value: step.target };
    }
  }
    ...
    switch (action) {
      ...
      case "fill":
        if (step.value != null) {
          ...
          const placeholderCount = await placeholderLocator
            .count()
            .catch(() => 0);
          if (placeholderCount === 1) {
            ...
            return {
              ...
              debug: debugBase
                ? {
                    ...debugBase,
                    fallbackLocators: resolvedLocator.value
                      ? [`${resolvedLocator.source}:${resolvedLocator.value}`]
                      : undefined,
            ...
            await waitForReplayLocator(resolvedLocator.locator, timeoutMs);
            await resolvedLocator.locator.click({ timeout: timeoutMs });
            await resolvedLocator.locator.fill(step.value, {
              timeout: timeoutMs,
            });
          }
        }
