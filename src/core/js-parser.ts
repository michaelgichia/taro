...
export function segmentIntoItGroups(steps: NormalizedStep[]): ItGroup[] {
  ...
  if (current.length > 0) {
    groups.push({
      name: current[0]?.target ?? "recorded flow",
      steps: current,
    });
  }
...
function isRecorderTitleCall(node: t.CallExpression): boolean {
  return (
    t.isIdentifier(node.callee) && ["test", "it"].includes(node.callee.name)
  );
}
...
function fallbackDocCommentTitle(code: string): string | undefined {
  ...
  return title
    ? title.replace(/\s+at\s+\d{1,2}:\d{2}:\d{2}/, "").replace(/-/g, " ")
    : undefined;
}
...
function extractSelectorDescriptor(
  ...
  return {
    ...
    selector,
    selectorKind:
      `document.${node.callee.property.name}` as SelectorDescriptor["selectorKind"],
    line: getLine(node),
    raw: sliceSource(code, node),
  };
}
...
function resolveTarget(
  ...
  if (t.isCallExpression(node)) {
    ...
    if (selector) {
      return { selector, target: selector.selector, metadata: { selector } };
    }
  }
...
function looksLikeVisibleMessage(value?: string): boolean {
  ...
  return /^(please|saved|success|error|warning|failed|updated|deleted)\b/i.test(
    value
  );
}
...
function classifySemanticMarkerProofSubject(
  ...
  const proofText = normalizeProofText(
    resolvedTarget.query?.target ?? resolvedTarget.target
  );
  ...
  if (
    resolvedTarget.query?.role === "alert" ||
    resolvedTarget.query?.role === "status"
  ) {
    return "visible-message";
  }

  if (
    isDisplayValueQueryMethod(resolvedTarget.query?.method) ||
    looksLikeConcreteValue(proofText)
  ) {
...
export async function parseJsRecording(code: string): Promise<JsParseResult> {
  ...
  const resolvedTitle =
    title ?? fallbackDocCommentTitle(code) ?? "Recorded Flow";
