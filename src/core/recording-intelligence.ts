...
function isResolvableFieldContextCandidate(
  ...
  if (
    isLabelTextQueryMethod(queryMethod) ||
    isPlaceholderTextQueryMethod(queryMethod)
  ) {
    return true;
  }

  return (
    isTextQueryMethod(queryMethod) && FIELD_LABEL_HINT_PATTERN.test(proofText)
  );
}
...
function deriveIntentLabel(steps: NormalizedStep[]): string {
  ...
  const navigateStep = meaningfulSteps.find(
    (step) => step.action === "navigate"
  );
  if (navigateStep) {
    return navigateStep.target
      ? `navigate to ${navigateStep.target}`
      : "navigation flow";
  }
...
function findGroupSelector(group: IntentGroup): string | undefined {
  return group.steps.find((step) => step.action !== "navigate" && step.target)
    ?.target;
}
...
export function filterNoiseSteps(steps: NormalizedStep[]): NoiseFilterResult {
  ...
  for (let index = 0; index < annotatedSteps.length; index += 1) {
    ...
    const markerGestures = cluster.filter((candidate) =>
    ...
    if (preservedMarkers.length > 0 || markerGestures.length > 0) {
      ...
      const markerIsInteractive = [...preservedMarkers, ...markerGestures].some(
        (candidate) => isInteractiveTarget(candidate)
      );
