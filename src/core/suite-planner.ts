...
function buildCompanionAnnotations(
...
function buildCompanionScenarioName(params: {
  ...
  return state === "in-flight"
    ? `${baseName} shows in-flight UI`
    : `${baseName} shows failure UI`;
}

function synthesizeContractCompanionScenarios(params: {
  scenarios: JsScenarioPlan[];
  contracts: JsDetectedInteractionContract[];
}): { scenarios: JsScenarioPlan[]; warnings: string[] } {
  ...
  const primarySteps =
    submitStepIndex >= 0
      ? primaryScenario.steps.slice(0, submitStepIndex + 1)
      : primaryScenario.steps;

  const synthesized = mutationContract.companionStates.map((state) => ({
    ...primaryScenario,
    name: buildCompanionScenarioName({ baseName: primaryScenario.name, state }),
    steps: primarySteps,
  ...
  return { scenarios: [...scenarios, ...synthesized], warnings };
}
