...
describe("scanMockTargets", () => {
  it("detects repeated mock targets across discovered tests", async () => {
    ...
    await writeFile(join(testDir, "beta.test.ts"), `jest.mock('./api/orders')`);
    ...
    expect(targets).toEqual([
      ...
      { target: "./api/users", files: ["alpha.test.ts"], count: 1 },
    ]);
  });
});
...
describe("analyzeMocks", () => {
  it("combines conventions and repeated-target analysis", async () => {
    ...
    expect(analysis.recommendations[0]).toEqual(
      expect.objectContaining({ target: "./api/orders", kind: "extract" })
    );
        ...
        repeatedMockTargets: [
          {
            ...
            files: [
              "packages/example-app/src/a.test.tsx",
              "packages/example-app/src/b.test.tsx",
            ],
            count: 2,
          },
          {
