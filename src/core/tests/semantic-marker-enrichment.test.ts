...
describe('enrichCanonicalSemanticMarkers', () => {
  ...
  it('enriches baseline semanticMarkerCandidates alongside steps', async () => {
    ...
    expect(enriched.steps[0]?.semanticMarkerCandidate?.proofText).toBe('Please enter or select an item')
    expect(enriched.baseline?.semanticMarkerCandidates?.[0]?.proofText).toBe('Please enter or select an item')
    expect(enriched.baseline?.itGroups[0]?.steps[0]?.semanticMarkerCandidate?.proofText).toBe(
  ...
  it('uses candidate.target as the recovery fragment when query fields are absent', async () => {
    ...
    expect(enriched.steps[1]?.semanticMarkerCandidate?.proofText).toBe('Please enter or select an item')
  })
  ...
  it('leaves baseline candidates untouched when only the recovered step id is valid', async () => {
    ...
    expect(enriched.steps[0]?.semanticMarkerCandidate?.proofText).toBe('Please enter or select an item')
    expect(enriched.baseline?.semanticMarkerCandidates?.[1]?.proofText).toBe('ignored')
    expect(
      enriched.baseline?.itGroups[0]?.steps[0]?.metadata?.semanticMarkerCandidate?.proofText
    ).toBe('ignored')
  })
  ...
  it('preserves baseline candidates when recovery cannot rebuild their query shape', async () => {
    ...
    expect(enriched.steps[0]?.semanticMarkerCandidate?.proofText).toBe('Please enter or select an item')
    expect(enriched.baseline?.semanticMarkerCandidates?.[0]?.proofText).toBe('Please enter or')
    expect(enriched.baseline?.semanticMarkerCandidates?.[0]?.query?.method).toBe('getByTestId')
    expect(enriched.baseline?.itGroups[0]?.steps[0]?.semanticMarkerCandidate?.proofText).toBe(
      'Please enter or'
    )
  })
