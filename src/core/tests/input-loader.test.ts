...
import { detectInputSource, loadInput } from "#core/input-loader.ts";
...
describe("detectInputSource", () => {
  it("rejects Chrome Recorder JSON by extension", () => {
    expect(() =>
      detectInputSource("/tmp/recording.json", '{"steps":[]}')
    ).toThrow("Chrome Recorder JSON exports are no longer supported.");
  });
...
describe("loadInput", () => {
  it("rejects JSON input through the shared parsed-input envelope", async () => {
    const filePath = await writeTempFile(
      ...
      JSON.stringify({
        ...
        steps: [{ type: "click", selectors: [["aria/Add sale"]] }],
      })
    );
  ...
  it("loads recorder JS input with preserved baseline metadata", async () => {
    ...
    expect(parsed.recording.steps[0]).toEqual(
      expect.objectContaining({ id: "js-step-1", source: "js" })
    );
