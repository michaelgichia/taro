  ...
  it('collects conditional counts, handler counts, import references, and exported utilities', async () => {
    ...
    expect(context?.componentImportReferences).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ target: '@/hooks/useProfileQuery', kind: 'hook' }),
        expect.objectContaining({ target: '@/lib/formatStatus', kind: 'helper' }),
        expect.objectContaining({ target: 'public/images/kenya-flag.svg', kind: 'asset' }),
        expect.objectContaining({
