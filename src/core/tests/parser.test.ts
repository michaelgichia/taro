...
describe('normalizeStep', () => {
  ...
  it('warns for known no-op and unknown step types', () => {
    ...
    try {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Unknown step type'))
    } finally {
      warnSpy.mockRestore()
    }
  })
...
describe('parseRecording', () => {
  ...
  it('throws for invalid JSON and invalid recorder shapes', async () => {
    ...
    const invalidShapePath = await writeTempFile('shape.json', JSON.stringify({ title: 'Flow' }))
    const invalidStepsPath = await writeTempFile('steps.json', JSON.stringify({ steps: 'bad' }))

    await expect(parseRecording(invalidJsonPath)).rejects.toThrow('Invalid JSON')
    await expect(parseRecording(invalidShapePath)).rejects.toThrow('missing required "steps" field')
    await expect(parseRecording(invalidStepsPath)).rejects.toThrow('"steps" must be an array')
  })
