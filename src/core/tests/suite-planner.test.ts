...
describe('planJsSuite', () => {
  ...
  it('emits a boundary draft warning when renderBoundary is not component and resolvedTarget is null', () => {
    ...
    const recording = createRecording([
      { action: 'click', target: 'Open wizard', originalType: 'click', source: 'js' },
      { action: 'fill', target: 'Customer', value: 'Alice', originalType: 'fill', source: 'js' },
      { action: 'fill', target: 'Email', value: 'a@b.com', originalType: 'fill', source: 'js' },
      { action: 'click', target: 'Continue', originalType: 'click', source: 'js' },
      { action: 'fill', target: 'Notes', value: 'hi', originalType: 'fill', source: 'js' },
      { action: 'click', target: 'Review', originalType: 'click', source: 'js' },
      { action: 'click', target: 'Save', originalType: 'click', source: 'js' },
    ])
  ...
  it('emits a repeated-target reuse warning when mockAnalysis has a repeated boundary target', () => {
    const recording = createRecording([
      { action: 'fill', target: 'Search', value: 'test', originalType: 'fill', source: 'js' },
      { action: 'click', target: 'Apply', originalType: 'click', source: 'js' },
    ])
    ...
    const plan = planJsSuite({
      recording,
      analyzedRecording: createAnalyzedRecording(recording, [{ name: 'search', steps: recording.steps }]),
      mockAnalysis,
      fallbackTitle: recording.title,
    })

    expect(plan.warnings.some((warning) => warning.includes('@repo/api-client'))).toBe(true)
  })
  ...
  it('detects interaction contracts from repo-signal when no explicit contracts exist but mutation lifecycle stages are present', () => {
    ...
    const plan = planJsSuite({
      recording,
      analyzedRecording: createAnalyzedRecording(recording, [{ name: 'save', steps: recording.steps }]),
      mockAnalysis,
      fallbackTitle: recording.title,
    })
  ...
  it('returns empty helperRefs for a wizard-flow scenario whose steps share no helpers (line 713)', () => {
    ...
    const normalStep = { id: 's1', action: 'click' as const, target: 'Step1', originalType: 'click', source: 'js' as const }
      ...
      steps: [
        {
          ...
          semanticMarkerCandidate: {
            ...
            target: 'MarkerTarget',
            sourceContext: { originalType: 'dblClick' },
            query: { stepId: 'marker-only', method: 'getByRole', queryRoot: 'screen', role: 'heading', target: 'MarkerTarget' },
            anchor: {},
          },
        },
      ],
    ...
    const markerScenario = plan.scenarios.find((s) => s.name === 'marker-only-group')
  ...
  it('detects repo-signal contract as medium confidence when boundaryProfiles has a supported strategy (lines 390-392, 404)', () => {
    ...
    expect(plan.contracts[0]?.evidence).toContain('shared boundary support signals detected')
  })
  ...
  it('resolves unresolved marker ONLY from metadata.unresolvedSemanticMarker (no candidate, lines 147-152)', () => {
    ...
    const foundMarker = unresolved.find((m) => m.markerStepId === 'meta-only-marker')
  ...
  it('returns no contracts when there is form input but no submit-like step', () => {
    ...
    const plan = planJsSuite({
      recording,
      analyzedRecording: createAnalyzedRecording(recording, [{ name: 'fill name', steps: recording.steps }]),
      mockAnalysis: createMockAnalysis(),
      fallbackTitle: recording.title,
    })
  ...
  it('does not synthesize companion scenarios when no mutation-form contract is present', () => {
    ...
    expect(plan.scenarios[0]?.provenance).toBe('recorded')
    expect(
      plan.warnings.some((warning) => warning.includes('companion'))
    ).toBe(false)
  })

  it('emits a warning and skips synthesis when high/medium contract exists but no scenario has a submit step', () => {
    ...
    const fillStep = { id: 'js-step-1', action: 'fill' as const, target: 'Name', value: 'Acme', originalType: 'fill', source: 'js' as const }
    const saveStep = { id: 'js-step-2', action: 'click' as const, target: 'Save', originalType: 'click', source: 'js' as const }
    const nonSubmitStep = { id: 'js-step-3', action: 'click' as const, target: 'Open', originalType: 'click', source: 'js' as const }
    ...
    const plan = planJsSuite({
      recording,
      analyzedRecording: createAnalyzedRecording(recording, [{ name: 'open', steps: [nonSubmitStep] }]),
      mockAnalysis,
      fallbackTitle: recording.title,
    })
  ...
  it('adds boundary-profile signal to assessRenderBoundary when profiles are present', () => {
    ...
    const plan = planJsSuite({
      recording,
      analyzedRecording: createAnalyzedRecording(recording, [{ name: 'open', steps: recording.steps }]),
      mockAnalysis,
      fallbackTitle: recording.title,
    })

    expect(plan.renderBoundary.signals).toContain('repo already documents boundary support patterns')
  })
  ...
  it('marks unresolved marker assertions with boundary-placement-conflict when anchor step is in multiple scenarios', () => {
    ...
    const allUnresolved = plan.scenarios.flatMap((s) => s.unresolvedMarkerAssertions)
    expect(allUnresolved.length).toBeGreaterThan(0)
    expect(allUnresolved[0]).toMatchObject({ reason: 'boundary-placement-conflict' })
  })
  ...
  it('uses after-step placement when the anchor step is not present in any helper (lines 799-801)', () => {
    ...
    const intentGroups: ItGroup[] = [
      { name: 'scenario-a-with-anchor', steps: [anchorAsMarkerStep, markerStep2] },
    ]
    ...
    const allAssertions = plan2.scenarios.flatMap((s) => s.markerAssertions)
  ...
  it('marks unresolved-status marker with anchorStepId in multiple groups as boundary-placement-conflict (lines 766-770)', () => {
    ...
    const allUnresolved = plan.scenarios.flatMap((s) => s.unresolvedMarkerAssertions)
    expect(allUnresolved.length).toBeGreaterThan(0)
    const conflictMarker = allUnresolved.find((m) => m.reason === 'boundary-placement-conflict')
  ...
  it('uses after-step placement when getHelperPlacement returns null (anchor not in any helper)', () => {
    // To get after-step: scenario must have helperRefs but the anchor step must not be
    // present in any of those helpers. This happens when anchor is a managed semantic marker step
    // (filtered out of helper steps) but is still in the scenario group.
    const anchorStep: NormalizedRecording['steps'][0] = {
      id: 'anchor-step',
      action: 'click',
    ...
    const otherStep: NormalizedRecording['steps'][0] = {
      id: 'other-step',
      action: 'click',
      target: 'OtherAction',
      originalType: 'click',
    ...
    const markerStep: NormalizedRecording['steps'][0] = {
      ...
      semanticMarkerCandidate: {
        ...
        anchor: { anchorStepId: 'anchor-step', relation: 'precedes' },
      },
    }

    const recording = createRecording([anchorStep, otherStep, markerStep])

    // Two groups: group-1 has anchorStep + marker, group-2 has otherStep
    // anchorOwners for 'anchor-step' = [0] (only in group-1)
    // scenario-1 (group-1) helperRefs = [helper-for-group-1, helper-for-group-2] if setup-replay
    //   OR just [helper-for-group-1] in safe-multi-it (shares steps)
    // helper for group-1 = filterManagedSemanticMarkerSteps([anchorStep, markerStep])
    //   = [anchorStep] (markerStep is managed → filtered out)
    // helper has 'anchor-step' in its step IDs → getHelperPlacement WILL find it → after-helper

    // To get after-step: we need helper NOT to contain the anchor.
    // Scenario 2 (group-2) owns otherStep. If we create a marker whose anchor is 'other-step'
    // AND 'other-step' is NOT in helper-for-group-1 (which IS the case since group-2 has otherStep)
    // but scenario-2 references helper-for-group-2 which has otherStep → still after-helper.

    // The ONLY way to get after-step is if helperRefs is EMPTY for the target scenario.
    // Force this by making the scenarios have no shared steps with any helper.
    // In safe-multi-it, helperRefs = helpers that sharesAnyStep with the group.
    // If no helper shares any step with the scenario → helperRefs = [] → getHelperPlacement = null → after-step

    // A solo group with a unique anchor where no helper shares its steps:
    const soloStep: NormalizedRecording['steps'][0] = {
      id: 'solo-anchor',
      action: 'click',
      target: 'UniqueAction',
      originalType: 'click',
    ...
    const soloMarkerStep: NormalizedRecording['steps'][0] = {
      id: 'solo-marker',
      action: 'click',
      target: 'SoloReview',
      originalType: 'dblClick',
      ...
      semanticMarkerCandidate: {
        ...
        query: {
          ...
          queryRoot: 'screen',
          role: 'heading',
          raw: "screen.getByRole('heading', { name: 'SoloReview' })",
          target: 'SoloReview',
        },
        anchor: { anchorStepId: 'solo-anchor', relation: 'precedes' },
      },
    }

    const soloRecording = createRecording([soloStep, soloMarkerStep])

    // Single group with both steps. The helper for this group = [soloStep] (marker filtered).
    // The scenario's helperRefs = ['planSoloFlow'] (helper shares soloStep with scenario).
    // helperStepsByName for planSoloFlow = Set(['solo-anchor'])
    // getHelperPlacement for anchor 'solo-anchor' → found in planSoloFlow → after-helper placement.

    // So single group always → after-helper. Need TWO separate groups where anchor is in group 1,
    // marker is in group 2, and group 2's helperRefs does NOT include group 1's helper.
    // In safe-multi-it: scenario helperRefs = helpers that share steps with the scenario.
    // Group 2 has only marker step (managed → not in helper steps) → helper has no non-managed steps
    // → helperRefs = [helper-for-group-2] (which shares marker step... but marker IS managed so filtered)
    // Actually sharesAnyStep checks ALL steps including managed ones.

    // Group 2 steps = [soloMarkerStep]. sharesAnyStep(group2.steps, helper.steps):
    //   helper-for-group-2 steps = filterManagedSemanticMarkerSteps([soloMarkerStep]) = []
    //   helper-for-group-2 has no steps → sharesAnyStep([soloMarkerStep], []) = false
    //   BUT helper itself was built from group.steps = [soloMarkerStep]
    //   Wait: helpers use filterManagedSemanticMarkerSteps(group.steps) for the helper's steps
    //   but sharesAnyStep is called between group.steps and helper.steps (filtered)
    //   group2.steps = [soloMarkerStep], helper-for-group-2.steps = [] → NO match
    //   helperRefs for group-2 = []

    // So a 2-group scenario where group-2 ONLY has managed marker steps:
    // - anchor is in group-1 → anchorOwners = [0]
    // - targetScenario = scenario-0 (group-1)
    // - scenario-0 helperRefs: helpers sharing steps with group-1. Group-1 has [soloStep].
    //   helper-for-group-1 has [soloStep] → sharesAnyStep([soloStep], [soloStep]) = true
    //   helper-for-group-2 has [] → sharesAnyStep([soloStep], []) = false
    //   helperRefs = ['planGroup1']
    // - helperStepsByName for planGroup1 = Set(['solo-anchor'])
    // - getHelperPlacement(anchorStepId='solo-anchor', helperRefs=['planGroup1'], ...) → finds it → after-helper

    // Hmm. Let me try: make anchor step have NO id, so it won't be in the helperStepsByName set.
    const noIdAnchorStep: NormalizedRecording['steps'][0] = {
      // no id!
      action: 'click',
      target: 'NoIdAction',
      originalType: 'click',
    ...
    const noIdMarkerStep: NormalizedRecording['steps'][0] = {
      id: 'noIdMarker',
      action: 'click',
      target: 'NoIdReview',
      originalType: 'dblClick',
      ...
      semanticMarkerCandidate: {
        ...
        query: {
          ...
          queryRoot: 'screen',
          role: 'heading',
          raw: "screen.getByRole('heading', { name: 'NoIdReview' })",
          target: 'NoIdReview',
        },
        anchor: { anchorStepId: 'some-external-anchor', relation: 'precedes' },
      },
    }

    const noIdRecording = createRecording([noIdAnchorStep, noIdMarkerStep])
    // anchorStepId = 'some-external-anchor' — NOT in any group's steps
    // anchorOwners = scenarioOwnersByStepId.get('some-external-anchor') ?? [] = []
    // anchorOwners.length !== 1 → goes to the `anchorOwners.length !== 1` path at line 779
    // This doesn't help for after-step.

    // Actually the simplest path to after-step (lines 799-801):
    // targetScenario.helperRefs is non-empty BUT anchor is not in any of those helpers.
    // This is guaranteed when:
    //   - targetScenario has helperRefs (e.g., ['planGroupOne'])
    //   - helperStepsByName.get('planGroupOne') does NOT have the anchorStepId
    // This happens when the anchor step has NO id (so it's not added to helperStepsByName)

    // anchor step has no id → not in helperStepsByName → getHelperPlacement returns null → after-step!
    const rec3 = createRecording([noIdAnchorStep, noIdMarkerStep])
    const groups3: ItGroup[] = [
      { name: 'group-one', steps: [noIdAnchorStep, noIdMarkerStep] },
    ]

    // But wait, if anchor has no id, the marker's anchorStepId = 'some-external-anchor'
    // won't be in scenarioOwnersByStepId either → anchorOwners = [] → that's NOT 1 → goes to line 779

    // The actual after-step path (lines 799-801) requires anchorOwners.length === 1 but
    // getHelperPlacement returns null. So I need:
    //   1. anchor step HAS an id and IS in exactly one scenario group
    //   2. targetScenario.helperRefs is non-empty (otherwise we'd just use default placement)
    //   3. The anchor step is NOT in any helper's step set
    //
    // Condition 3: the anchor step IS managed (isManagedSemanticMarkerStep = true) so it gets
    // filtered from helper steps. But a managed step won't have 'click' action to be counted
    // in scenario ownership... actually scenario ownership is based on group.steps which includes
    // ALL steps including managed ones.
    ...
    const rec4 = createRecording([normalHelperStep, managedAnchorStep, markerWithManagedAnchor])
    ...
    const groups4: ItGroup[] = [
      { name: 'group zero', steps: [normalHelperStep, managedAnchorStep, markerWithManagedAnchor] },
    ]
    ...
    const afterStepAssertion = assertions.find((a) => a.placement.kind === 'after-step')
