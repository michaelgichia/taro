...
describe('convention-intelligence', () => {
  it('skips unreadable directories and ignored roots while finding tests', async () => {
    ...
    await writeFile(join(root, 'src', '__tests__', 'good.test.tsx'), 'describe("x",()=>{})', 'utf8')
    await writeFile(join(root, '.git', 'bad.test.ts'), 'describe("x",()=>{})', 'utf8')
  ...
  it('reads test files while dropping unreadable entries and handles missing files', async () => {
    ...
    await writeFile(join(root, 'tests', 'first.test.ts'), 'vi.mock("x")', 'utf8')

    const loaded = await readTestFiles(root)
    const missing = await analyzeTestFile(join(root, 'tests', 'missing.test.ts'))
