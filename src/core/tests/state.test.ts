...
describe("initTaroState", () => {
  ...
  it("builds package-scoped runner profiles for mixed workspaces", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify(
        { name: "@repo/example-app", devDependencies: { vitest: "^3.0.0" } },
        null,
        2
      ),
      "utf-8"
    ...
    await writeFile(
      ...
      JSON.stringify(
        { name: "@repo/legacy", devDependencies: { jest: "^29.0.0" } },
        null,
        2
      ),
      "utf-8"
    ...
    await writeFile(
      join(vitestPackage, "vitest.config.ts"),
      "export default {}",
      "utf-8"
    );
    await writeFile(
      join(jestPackage, "jest.config.ts"),
      "module.exports = {}",
      "utf-8"
    );
    await writeFile(
    ...
    expect(result.state.packages["packages/example-app"]?.runner.value).toBe(
      "vitest"
    );
  ...
  it("learns global jest-dom setup from configured Vitest setup files", async () => {
    ...
    expect(result.state.packages["packages/example-app"]?.jestDomSetup).toEqual(
      expect.objectContaining({ value: "global-setup", confidence: "high" })
    );
  });
  ...
  it("assigns unmatched root-level tests to the nearest available package when no root descriptor exists", async () => {
    ...
    expect(Object.keys(result.state.packages)).toEqual([
      "packages/example-app",
    ]);
    expect(result.state.packages["packages/example-app"]?.testFileCount).toBe(
      1
    );
  });
  ...
  it("learns render helpers, repeated mocks, fixture roots, and respects overrides", async () => {
    const examplePackage = join(projectRoot, "packages", "example-app");
    await mkdir(join(examplePackage, "src", "tests", "mock-store"), {
      recursive: true,
    });
    await writeFile(
      join(examplePackage, "package.json"),
    ...
    expect(exampleProfile?.repeatedMockTargets).toEqual([
      {
        ...
        files: [
          "packages/example-app/src/checkout.test.tsx",
          "packages/example-app/src/feature-flow.test.tsx",
        ],
        count: 2,
      },
    ]);
    ...
    expect(result.state.mockStore.rootDir).toBe(
      "packages/example-app/src/tests/mock-store"
    );
  ...
  it("learns boundary profiles and writes a human-readable boundary summary", async () => {
    const examplePackage = join(projectRoot, "packages", "example-app");
    await mkdir(join(examplePackage, "src", "tests", "mocks"), {
      recursive: true,
    });
    await writeFile(
      join(examplePackage, "package.json"),
    ...
    expect(summary).toContain(
      "- Collaborator categories: data-module=1, local-child=1"
    );
  ...
  it("detects Playwright storageState assets from config during init", async () => {
    ...
    expect(
      result.state.packages["packages/example-app"]?.playwrightAuth
    ).toEqual({
...
describe("loadOrBootstrapTaroState", () => {
  ...
  it("migrates compatibility .taro convention and history files into .taro/state.json", async () => {
    ...
    await expect(
      readFile(join(projectRoot, ".taro", "state.json"), "utf-8")
    ).resolves.toContain('"generatedTests"');
  });
});
...
describe("state hardening", () => {
  it("detects stale package profiles when package evidence changes after scan", async () => {
    ...
    expect(staleness.reason).toContain(
      "changed after the package profile was scanned"
    );
  });
  ...
  it("preserves manually persisted playwright auth across refresh", async () => {
    const examplePackage = join(projectRoot, "packages", "example-app");
    await mkdir(join(examplePackage, "src"), { recursive: true });
    await mkdir(join(projectRoot, ".taro", "playwright", ".auth"), {
      recursive: true,
    });
    await writeFile(
      join(examplePackage, "package.json"),
    ...
    await mkdir(join(examplePackage, "playwright", ".auth"), {
      recursive: true,
    });
    await writeFile(
    ...
    expect(
      refreshed.state.packages["packages/example-app"]?.playwrightAuth
    ).toEqual({
      strategy: "storageState",
      path: ".taro/playwright/.auth/user.json",
      detectedAt: "generate",
...
describe("findRepoFallbackPackageProfile", () => {
  ...
  it("returns the package with the most test files when there is no root package", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify(
        { name: "@repo/alpha", devDependencies: { vitest: "^3.0.0" } },
        null,
        2
      ),
      "utf-8"
    ...
    await writeFile(
      ...
      JSON.stringify(
        { name: "@repo/beta", devDependencies: { vitest: "^3.0.0" } },
        null,
        2
      ),
      "utf-8"
    ...
    await writeFile(
      join(pkgA, "vitest.config.ts"),
      "export default {}",
      "utf-8"
    );
    await writeFile(
      join(pkgB, "vitest.config.ts"),
      "export default {}",
      "utf-8"
    );
    await writeFile(
...
describe("formatStateSummary", () => {
  it("formats an init summary with package details and warnings", async () => {
    ...
    expect(lines.some((line) => line.includes("packages/example-app"))).toBe(
      true
    );
  });
  ...
  it("includes migration notice when legacy state was migrated", () => {
    ...
    expect(
      lines.some((line) => line.includes("consolidated compatibility"))
    ).toBe(true);
  });
  ...
  it("includes overrides notice when overrides are applied", async () => {
    ...
    await writeFile(
      join(projectRoot, ".taro", "overrides.json"),
      JSON.stringify(
        { packages: { "packages/example-app": { runner: "jest" } } },
        null,
        2
      ),
      "utf-8"
  ...
  it("includes warnings in the summary output", async () => {
    ...
    expect(lines.some((line) => line.includes("Something went wrong."))).toBe(
      true
    );
  });
});
  ...
  it("appends a generated test record to state and persists it", async () => {
    ...
    expect(state?.generatedTests[0]).toEqual(
      expect.objectContaining({
        ...
        recordingFile: "/tmp/recording.js",
        testFile: "/tmp/recording.test.tsx",
        quality: expect.objectContaining({ overall: 85, grade: "B" }),
        requiresReview: false,
      })
    );
  });
...
describe("persistPlaywrightAuthProfile", () => {
  it("returns false when the target package does not exist in state", async () => {
    ...
    const result = await persistPlaywrightAuthProfile(
      projectRoot,
...
describe("resolveTaroPackageProfile - override combinations", () => {
  ...
  it("applies forbidMocks override and records it in appliedOverrides", async () => {
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      projectRoot,
      join(examplePackage, "src", "app.test.ts"),
      { packages: { "packages/example-app": { forbidMocks: ["@/utils/api"] } } }
    );
  ...
  it("applies preferredSharedMocks override and records it in appliedOverrides", async () => {
    ...
    expect(resolved?.preferredBoundaryImplementations["@/features/api"]).toBe(
      "@/tests/mocks/api"
    );
  });
  ...
  it("applies preferredBoundaryImplementations override", async () => {
    ...
    expect(resolved?.appliedOverrides).toContain(
      "preferredBoundaryImplementations"
    );
  });
  ...
  it("applies forbidBoundaryTargets override", async () => {
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      {
        packages: {
          "packages/example-app": { forbidBoundaryTargets: ["@/external/sdk"] },
        },
      }
    );
  ...
  it("applies queryHookPolicy override", async () => {
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      {
        packages: {
          "packages/example-app": { queryHookPolicy: "allow-centralized" },
        },
      }
    );
  ...
  it("applies companionPolicy override", async () => {
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      projectRoot,
      join(examplePackage, "src", "app.test.ts"),
      { packages: { "packages/example-app": { companionPolicy: "off" } } }
    );
  ...
  it("injects a new boundary profile from preferredBoundaryImplementations when the target does not already exist", async () => {
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      join(examplePackage, "src", "app.test.ts"),
      {
        packages: {
          "packages/example-app": {
            preferredBoundaryImplementations: {
              "@/services/payments": "@/tests/mocks/payments",
            },
          },
        },
      }
    ...
    const injected = resolved?.boundaryProfiles.find(
      (p) => p.target === "@/services/payments"
    );
    ...
    const resolved = resolveTaroPackageProfile(
      ...
      join(examplePackage, "src", "app.test.ts"),
      {
        packages: {
          "packages/example-app": {
            preferredBoundaryImplementations: {
              "@/api": "@/tests/mock-store/api-helpers",
            },
          },
        },
      }
...
describe("state scanning - additional coverage", () => {
  ...
  it("detects mock instability warnings (recreated-factory) from test files", async () => {
    ...
    expect(
      profile?.instabilityWarnings.some((w) => w.kind === "recreated-factory")
    ).toBe(true);
  });
  ...
  it("detects mock instability warnings (per-test-churn) from reset + reconfigure pattern", async () => {
    ...
    expect(
      profile?.instabilityWarnings.some((w) => w.kind === "per-test-churn")
    ).toBe(true);
  });
  ...
  it("skips unreadable mock-store files while preserving readable resources", async () => {
    ...
    await writeFile(
      join(mockStoreRoot, "orders.ts"),
      "export const ORDER_001 = 'ORDER_001'\n",
      "utf-8"
    );
    ...
    const result = await __stateTestUtils.collectMockStoreResources(
      projectRoot,
      {
        "packages/example-app": {
          fixtureRoots: [
  ...
  it("returns an empty mock-store inventory when the learned root is not a directory", async () => {
    ...
    await mkdir(join(projectRoot, "packages", "example-app", "src", "tests"), {
      recursive: true,
    });
    await writeFile(
      fakeRoot,
  ...
  it("keeps root-package fixture directories unprefixed and sorts render targets deterministically", async () => {
    await mkdir(join(projectRoot, "src", "tests", "mocks"), {
      recursive: true,
    });
    await writeFile(
      join(projectRoot, "src", "tests", "mocks", "orders.ts"),
    ...
    expect(profile.fixtureRoots).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ path: "src/tests/mocks", kind: "mocks" }),
      ])
    );
    expect(profile.renderTargets.map((target) => target.symbol)).toEqual([
      "AlphaPanel",
      "ZebraPanel",
    ]);
  });
  ...
  it("treats unreadable config roots and descriptor walks as absent instead of throwing", async () => {
    ...
    expect(
      await __stateTestUtils.hasConfigFile(packageJsonFile, "vitest.config.")
    ).toBe(false);
    await expect(
      __stateTestUtils.findPackageDescriptors(packageJsonFile)
    ).resolves.toEqual([{ key: ".", root: packageJsonFile, name: null }]);
  });

  it("ignores unreadable and skipped directories while scanning fixture roots", async () => {
    const packageJsonFile = join(projectRoot, "package.json");
    await mkdir(join(projectRoot, "src", "tests", "fixtures"), {
      recursive: true,
    });
    await mkdir(join(projectRoot, "node_modules", "fixtures"), {
      recursive: true,
    });
    await mkdir(
      join(projectRoot, "a", "b", "c", "d", "e", "f", "g", "fixtures"),
      { recursive: true }
    );

    await expect(
      __stateTestUtils.collectFixtureDirs(packageJsonFile)
    ).resolves.toEqual([]);
    await expect(
      __stateTestUtils.collectFixtureDirs(projectRoot)
    ).resolves.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("derives low-confidence interaction contracts, sorts instability warnings, and infers extension confidence", () => {
    ...
    ).toEqual(expect.objectContaining({ value: "tsx", confidence: "medium" }));
    ...
    ).toEqual(expect.objectContaining({ value: "ts", confidence: "high" }));
  ...
  it("builds shared mock factory profiles from test files that import from mock/fixture paths", async () => {
    ...
    expect(
      profile?.sharedMockFactories.some((f) => f.target === "createOrderMock")
    ).toBe(true);
    const factory = profile?.sharedMockFactories.find(
      (f) => f.target === "createOrderMock"
    );
  ...
  it("detects provider wrappers used in wrapper: PropName syntax", async () => {
    ...
    expect(
      profile?.providerWrappers.some((w) => w.name === "TestQueryProvider")
    ).toBe(true);
  });
  ...
  it('writes summary with "No package-scoped test knowledge has been learned yet." when no packages', async () => {
    ...
    expect(summary).toContain(
      "No package-scoped test knowledge has been learned yet."
    );
  });
