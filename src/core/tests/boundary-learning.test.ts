...
describe("classifyBoundaryKind", () => {
  ...
  it("classifies feature flag paths", () => {
    expect(classifyBoundaryKind("launchdarkly-react-client-sdk")).toBe(
      "feature-flag"
    );
  ...
  it("classifies server action paths", () => {
    ...
    expect(classifyBoundaryKind("@/server-actions/submit")).toBe(
      "server-action"
    );
  ...
  it("normalizes backslashes before classifying", () => {
    expect(classifyBoundaryKind(".\\actions\\createOrder")).toBe(
      "server-action"
    );
    expect(classifyBoundaryKind(".\\hooks\\useRouter")).toBe("router");
  });
...
describe("getBoundaryGuardrailReason", () => {
  ...
  it("returns repo-owned-ui-wrapper for repo-owned UI paths with component exports", () => {
    expect(
      getBoundaryGuardrailReason("@/components/library/Modal", [
        "Dialog",
        "DialogContent",
      ])
    ).toBe("repo-owned-ui-wrapper");

    expect(getBoundaryGuardrailReason("./components/Button", ["Button"])).toBe(
      "repo-owned-ui-wrapper"
    );

    expect(getBoundaryGuardrailReason("@/ui/Card", ["Card"])).toBe(
      "repo-owned-ui-wrapper"
    );
  });

  it("returns ui-package for third-party UI packages", () => {
    expect(getBoundaryGuardrailReason("@radix-ui/components", ["Dialog"])).toBe(
      "ui-package"
    );
    expect(getBoundaryGuardrailReason("@shadcn/ui", ["Button"])).toBe(
      "ui-package"
    );
  });
...
describe("discoverBoundaryImportsFromSource", () => {
  it("returns empty array when file does not exist", async () => {
    const result = await discoverBoundaryImportsFromSource(
...
`
    ...
    expect(result.some((r) => r.target.startsWith("@testing-library/"))).toBe(
      false
    );
...
describe("__boundaryLearningTestUtils", () => {
  ...
  it("reads static template literal strings and returns null for unsupported bindings", () => {
    const templateLiteral = t.templateLiteral(
      [
        t.templateElement(
          { cooked: "./api/orders", raw: "./api/orders" },
          true
        ),
      ],
      []
    ...
    expect(__boundaryLearningTestUtils.getStringLiteral(templateLiteral)).toBe(
      "./api/orders"
    );
    ...
    expect(
      __boundaryLearningTestUtils.getStringLiteral(
        t.identifier("dynamicTarget")
      )
    ).toBeNull();
    expect(
      __boundaryLearningTestUtils.inferPayloadSource("../support/ordersPayload")
    ).toBe("manual");
    expect(__boundaryLearningTestUtils.strategyPriority("inline-safe")).toBe(2);
    expect(__boundaryLearningTestUtils.strategyPriority("real-runtime")).toBe(
      1
    );
    expect(
      __boundaryLearningTestUtils.isComponentLikeExportName("useDialog")
    ).toBe(false);
    expect(
      __boundaryLearningTestUtils.isComponentLikeExportName("DIALOG")
    ).toBe(false);
  });
});
...
`;
    ...
    const ordersProfile = result.profiles.find(
      (p) => p.target === "./api/orders"
    );
    expect(ordersProfile?.strategy).toBe("shared-module-factory");
    expect(ordersProfile?.supportExports.factoryExport).toBe(
      "createOrdersMock"
    );
  });
...
`;
    ...
    const modalProfile = result.profiles.find(
      (p) => p.target === "@/components/ui/modal"
    );
...
`;
    ...
    const routerProfile = result.profiles.find(
      (p) => p.target === "next/navigation"
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("override.test.ts")
    );
...
`;
    ...
    expect(ordersProfile?.supportExports.spyExports).toContain(
      "createOrderSpy"
    );
  });
  ...
  it("includes provider-wrapper observation from providerWrappers param", async () => {
    ...
    const wrapperProfile = result.profiles.find(
      (p) => p.target === "@/providers/AppWrapper"
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("mutation.test.ts")
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("module.test.ts")
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("within.test.ts")
    );
    expect(exemplar?.renderBoundary).toBe("module");
  });
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("unknown-render.test.ts")
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("wrapper.test.tsx")
    );
...
`;
    ...
    const exemplar = result.exemplars.find((e) =>
      e.file.includes("central.test.ts")
    );
...
`;
    ...
    const exemplar = result.exemplars.find(
      (e) => e.file === "src/tagged-wrapper.test.tsx"
    );
    expect(exemplar?.usesProviderWrapper).toBe(true);
...
`;
    ...
    const result = await collectBoundaryLearning({
      projectRoot: testDir,
      testFiles: [{ path: filePathA, content: contentA }],
      renderTargets: [],
      providerWrappers: [
        {
          name: "AppWrapper",
...
describe("summarizeBoundaryProfiles", () => {
  const baseOptions = { renderHelpers: [], playwrightAuth: null };
    ...
    const profiles: TaroBoundaryProfile[] = [
      {
        ...
        conflictTargets: [
          "real-runtime",
          "shared-module-factory -> ..//__mocks__/b",
        ],
        lowConfidenceScaffold: false,
      },
    ];
  ...
  it("appends playwright auth line when present", () => {
    ...
    expect(lines.some((l) => l.includes("playwright/.auth/user.json"))).toBe(
      true
    );
  });
