...
function createTarget(overrides: Partial<MockTarget> = {}): MockTarget {
  return {
    apiCallId: "call-1",
...
describe("buildMock", () => {
  ...
  it("builds a jest.fn mock and defaults unknown libraries to jest.fn", () => {
    const explicit = buildMock(
      createTarget({ mockLibrary: "jest.fn", method: "DELETE" }),
      createApiCall()
    );
    ...
    expect(explicit.code).toContain(
  ...
  it("builds sinon, fetch-mock, nock, and undici mocks with library-specific setup", () => {
    ...
    expect(fetchMockDecision.code).toContain(
  ...
  it("covers post/patch/default MSW handlers, fallback jest mocks, and invalid URL parsing", () => {
    ...
    const unknownJestDecision = buildMock(
      createTarget({ mockLibrary: "jest.fn", method: "OPTIONS" }),
      createApiCall()
    );
    ...
    expect(patchDecision.code).toContain(
      "http.patch('https://api.example.com/list'"
    );
    expect(deleteDecision.code).toContain(
...
describe("generateMockFile", () => {
  ...
  it("omits vitest hooks when generating a jest-oriented mock file", () => {
    const content = generateMockFile([
      buildMock(
        createTarget({ mockLibrary: "msw", extractionRecommendation: "file" }),
        createApiCall()
      ),
    ]);
...
describe("generateInlineMock", () => {
  it("renders inline imports, setup code, and teardown code", () => {
    ...
    expect(inlineCode).toContain(
      "// Setup mock for https://api.example.com/users"
    );
  ...
  it("returns setup-only inline code when the mock has no imports or teardown hints", () => {
    const inlineCode = generateInlineMock(
      buildMock(
        createTarget({ mockLibrary: "jest.fn", method: "POST" }),
        createApiCall()
      )
    );
