...
function createStep(overrides: Partial<RecordingStep> = {}): RecordingStep {
  return {
    id: "json-step-1",
    type: "click",
    action: "click",
...
function createFlow(overrides: Partial<DialogFlow> = {}): DialogFlow {
  return {
    id: "flow-1",
    type: "modal",
...
describe("transformDialogFlows", () => {
  ...
  it("builds modal, drawer, popover, confirm, and fallback dialog templates", () => {
    ...
    expect(templates[0].helpers[0].code).toContain(
    ...
    expect(templates[0].testSteps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "await openDialog();" }),
    ...
    expect(templates[1].helpers[0].code).toContain(
      'screen.getByTestId("open-drawer")'
    );
    expect(templates[1].helpers[0].code).toContain(
    ...
    expect(templates[2].helpers[0].code).toContain(
      "screen.getByRole('button', { name: /menu-trigger/i })"
    );
    expect(templates[2].helpers[0].code).toContain(
    ...
    expect(templates[3].helpers[0].code).toContain(
    ...
    expect(templates[4].helpers[0].code).toContain(
      "screen.getByRole('button', { name: /Dialog/i })"
    );
    expect(templates[4].helpers[0].code).toContain(
      "screen.getByRole('dialog')"
    );
...
describe("generateDialogTestCode", () => {
  it("renders helpers, steps, and cleanup blocks in order", () => {
    const code = generateDialogTestCode({
      ...
      testSteps: [
        { type: "action", code: "await openDialog();", description: "open" },
        {
          type: "assertion",
